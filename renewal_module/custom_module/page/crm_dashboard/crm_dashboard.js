...
/*function wait_for_element(selector, callback, max_attempts = 50, interval = 100) {
	let attempts = 0;
	const timer = setInterval(() => {
		...
		if ($el.length > 0 && $el.is(':visible')) {
			...
			clearInterval(timer);
			console.warn(`⚠️ Element ${selector} not found after ${max_attempts} attempts`);
		}
		attempts++;
	}, interval);
}*/
frappe.pages['crm-dashboard'].on_page_load = function (wrapper) {
	frappe._crm_dashboard_page = new Mypage(wrapper);  // ✅ Store instance globally
	if (frappe._crm_dashboard_page) {
		console.log("CRM Dashboard page loaded successfully");
	} else {
		console.error("Failed to initialize CRM Dashboard page");
	}
};

frappe.router.on('change', () => {
	const route = frappe.get_route();
	if (frappe._crm_dashboard_page && route[0] !== "crm-dashboard") {
...
class Mypage {
	...
	make() {
		...
		//target this month
		this.get_open_opportunity_amount();
		this.get_target_amount();
		this.get_achieved_amount();
	...
	/***hiddencharts(salesperson) {
		...
		if (!salesperson) {
			...
			// Expand targetCol and targetCategoryWiseChartCol to full width
			targetCol.classList.remove("col-md-9", "col-lg-9", "col-xl-10");
			targetCol.classList.add("col-12");

			targetCategoryWiseChartCol.classList.remove("col-md-12", "col-lg-7", "col-xl-8");
			targetCategoryWiseChartCol.classList.add("col-8 col-md-10 col-lg-8 col-xl-8");

			// Hide cards column
			cardscol.classList.add("col-2");
			cardscol.classList.remove("col-md-3", "col-lg-3", "col-xl-2");
		} else {
			// Show charts
			chartCol.style.display = "block";
			salesgrowthchartcol.style.display = "block";

			// Shrink targetCol back to its default layout
			targetCol.classList.remove("col-12");
			targetCol.classList.add("col-md-9", "col-lg-9", "col-xl-10");

			// Shrink targetCategoryWiseChartCol
	...
	/**add_tabs() {
		...
		this.add_tab_field = $(`
			<div class="dropdown ms-2">
				...
				<div class="dropdown-menu dropdown-menu-end">
					<a class="dropdown-item nav-link1 mb-2" id="review-tab" data-target="#review-content" role="tab" aria-controls="review-content" aria-selected="true">
					...
					<a class="dropdown-item nav-link1 mb-2" id="sales-stage-tab" data-target="#sales-stage-content" role="tab" aria-controls="sales-stage-content" aria-selected="false">
		...
		//$(".page-actions #dashboard-btn").closest(".dropdown").remove();
		// ✅ Append cleanly
		$(".page-actions").append(this.add_tab_field);
		// let $actions = this.get_visible_page_actions();
		// $actions.append(this.add_tab_field);

		// Retrieve the last active tab from localStorage
		let savedTab = localStorage.getItem("activeTab") || "review-tab"; // Default to "opp-tab" if not found
	...
	add_sales_person() {
		...
		// let $actions = this.get_visible_page_actions();
		// $actions.append(this.sales_person_field);

		//console.log("Dropdown should be added to:", $(".page-actions"));
	...
	update_dashboard_data(sales_person) {
		if (sales_person) {
			...
			this.get_closed_won_amount_tm(sales_person);
			this.get_closed_lost_amount_tm(sales_person);
			this.get_renewal_count_tm(sales_person);
			this.get_renewal_amount_tm(sales_person);
			this.get_profit_amount_tm(sales_person);
			this.sales_funnel_chart(sales_person);
			//renewal list
			this.get_rnwls_new_opp_count(sales_person);
			this.get_rnwls_active_count(sales_person);
			this.get_rnwls_cofed_count(sales_person);
			this.get_rnwls_lost_count(sales_person);
			this.get_rnwls_renewed_count(sales_person);
			this.get_rnwls_pending_count(sales_person);
			//closures this week
			...
			this.get_achieved_amount(sales_person);
	...
	get_leads_count(sales_person = null) {
		...
		$("#leads-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_leads_count",
	...
	get_quoted_count(sales_person = null) {
		...
		$("#quoted-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_quoted_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
	...
	get_demo_count(sales_person = null) {
		...
		$("#demo-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_demo_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
	...
	get_new_customer_count(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_new_customer_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
	...
	get_appointment_count(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_appointment_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
	...
	get_client_visit_count(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_client_visit_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
	...
	get_calls_count(sales_person = null) {
		...
		$("#calls-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_calls_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#calls-count").text(count);
				}
			}
		});
	}
	...
	get_training_count(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_training_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#training-count").text(count);
				}
			}
		});
	}
	...
	get_sessions_count(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_sessions_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#session-count").text(count);
				}
			}
		});
	}
	...
	get_opp_count_lw(sales_person = null) {
		...
		$("#opp-count-lw").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_opp_count_lw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#opp-count-lw").text(count);
				}
			}
		});
	}
	...
	get_closed_won_amount_lw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closed_won_amount_lw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#closed-won-amount-lw").text(formatted_price);
				}
			}
		});
	}
	...
	get_closed_lost_amount_lw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closed_lost_amount_lw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#closed-lost-amount-lw").text(formatted_price);
				}
			}
		});
	}
	...
	get_renewal_count_lw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_renewal_count_lw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					$("#renewal-count-lw").text(r.message);
				}
			}
		});
	}
	...
	get_renewal_amount_lw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_renewal_amount_lw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#renewal-amount-lw").text(formatted_price);
				}
			}
		});
	}
	...
	get_profit_amount_lw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_profit_amount_lw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#profit-amount-lw").text(formatted_price);
				}
			}
		});
	}
	...
	brand_wise_donut_chart_lw(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_brand_wise_data_lw",
				args: { sales_person: sales_person },
				callback: (r) => {
	...
	Item_group_donut_chart_lw(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_item_group_wise_data_lw",
				args: { sales_person: sales_person },
				callback: (r) => {
					if (r.message) {
	...
	get_opp_count_tm(sales_person = null) {
		...
		$("#opp-count-tm").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_opp_count_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#opp-count-tm").text(count);
				}
			}
		});
	}

	get_closed_won_amount_tm(sales_person = null) {
		$("#closed-won-amount-tm").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closed_won_amount_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#closed-won-amount-tm").text(formatted_price);
				}
			}
		});
	}

	get_closed_lost_amount_tm(sales_person = null) {
		$("#closed-lost-amount-tm").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closed_lost_amount_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#closed-lost-amount-tm").text(formatted_price);
				}
			}
		});
	}

	get_renewal_count_tm(sales_person = null) {
		$("#renewal-count-tm").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_renewal_count_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					//console.log("renewal count tm:", r.message);
					$("#renewal-count-tm").text(r.message);
				}
			}
		});
	}

	get_renewal_amount_tm(sales_person = null) {
		$("#renewal-amount-tm").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_renewal_amount_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#renewal-amount-tm").text(formatted_price);
				}
			}
		});
	}

	get_profit_amount_tm(sales_person = null) {
		$("#profit-amount-tm").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_profit_amount_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#profit-amount-tm").text(formatted_price);
				}
			}
		});
	}
	...
	sales_funnel_chart(sales_person = null) {
		//console.log("sales funnel", sales_person);
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_funnel_data",
				args: { sales_person: sales_person },
				callback: function (r) {
					//console.log("funnel data:", r.message);
	...
	get_rnwls_new_opp_count(sales_person = null) {
		//console.log("count sales person:", sales_person)
		$("#rnwls-quoted-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.rnwls_data.get_rnwls_new_opp_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#rnwls-new-opp-count").text(count);
				}
			}
		});
	}

	get_rnwls_active_count(sales_person = null) {
		//console.log("count sales person:", sales_person)
		$("#rnwls-active-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.rnwls_data.get_rnwls_active_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#rnwls-active-count").text(count);
				}
			}
		});
	}

	get_rnwls_cofed_count(sales_person = null) {
		//console.log("count sales person:", sales_person)
		$("#rnwls-cofed-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.rnwls_data.get_rnwls_cofed_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#rnwls-cofed-count").text(count);
				}
			}
		});
	}

	get_rnwls_lost_count(sales_person = null) {
		//console.log("count sales person:", sales_person)
		$("#rnwls-lost-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.rnwls_data.get_rnwls_lost_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#rnwls-lost-count").text(count);
				}
			}
		});
	}

	get_rnwls_renewed_count(sales_person = null) {
		//console.log("count sales person:", sales_person)
		$("#rnwls-renewed-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.rnwls_data.get_rnwls_renewed_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#rnwls-renewed-count").text(count);
				}
			}
		});
	}

	get_rnwls_pending_count(sales_person = null) {
		//console.log("count sales person:", sales_person)
		$("#rnwls-pending-count").text(0);
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.rnwls_data.get_rnwls_pending_count",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let count = r.message || 0;
					$("#rnwls-pending-count").text(count);
				}
			}
		});
	}
	...
	get_closure_opp_count_tw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closure_opp_count_tw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
	...
	get_closure_opp_amount_tw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closure_opp_amount_tw",
			args: { sales_person: sales_person },
			callback: function (r) {
	...
	get_closures_rnwls_count_tw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closures_rnwls_count_tw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					$("#closures-rnwls-count-tw").text(r.message);
				}
			}
		});
	}
	...
	get_closures_rnwls_amount_tw(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closures_rnwls_amount_tw",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#closures-rnwls-amount-tw").text(formatted_price);
				}
			}
		});
	}
	...
	brand_wise_donut_chart_tw(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_brand_wise_data_tw",
				args: { sales_person: sales_person },
				callback: (r) => {
					//console.log("brand wise data:", r.message);
					if (r.message) {
	...
	Item_group_donut_chart_tw(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_item_group_wise_data_tw",
				args: { sales_person: sales_person },
				callback: (r) => {
					if (r.message) {
	...
	get_closure_opp_count_tm(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closure_opp_count_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
	...
	get_closure_opp_amount_tm(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closure_opp_amount_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
	...
	get_closures_rnwls_count_tm(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closures_rnwls_count_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					$("#closures-rnwls-count-tm").text(r.message);
				}
			}
		});
	}
	...
	get_closures_rnwls_amount_tm(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_closures_rnwls_amount_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#closures-rnwls-amount-tm").text(formatted_price);
				}
			}
		});
	}
	...
	brand_wise_donut_chart_tm(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_brand_wise_data_tm",
				args: { sales_person: sales_person },
				callback: (r) => {
					//console.log("brand wise data:", r.message);
					if (r.message) {
	...
	Item_group_donut_chart_tm(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_item_group_wise_data_tm",
				args: { sales_person: sales_person },
				callback: (r) => {
					if (r.message) {
	...
	get_open_opportunity_amount(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_open_opportunity_amount_tm",
			args: { sales_person: sales_person },
			callback: function (r) {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#open-opportunity-amount").text(formatted_price);
				}
			}
		});
	}
	...
	get_target_amount(sales_person = null) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.crm_dashboard.get_target_amount_tm",
			args: { sales_person: sales_person },
			callback: (r) => {
				...
				if (r.message && r.message.bottomline_target !== undefined) {
					...
					$("#target-amount").text(formatted_price);
	...
	get_achieved_amount(sales_person = null) {
		...
		frappe.call({
			...
			callback: (r) => {
				if (r.message) {
					let formatted_price = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(r.message);
					$("#achieved-amount").text(formatted_price);
					this.get_variance_amount();
				}
			}
		});
	...
	sales_target_chart(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.target_data.get_target_data",
				args: { sales_person: sales_person },
				callback: (r) => {
	...
	salespersongrowthChart(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.target_data.get_sales_target_tm",
			args: { sales_person },
			callback: (r) => {
				if (r.message) {
	...
	salespersongrowthChartyear(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.target_data.get_sales_target_ty",
			args: { sales_person },
			callback: (r) => {
				if (r.message) {
	...
	target_category_wise_chart(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.target_data.target_category_wise_chart_tm",
				args: { sales_person: sales_person },
				callback: function (r) {
					if (r.message) {
	...
	target_category_wise_overall_chart(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.target_data.target_category_wise_overall_chart",
				args: { sales_person: sales_person },
				callback: function (r) {
					if (r.message) {
	...
	get_sales_target_tq(sales_person = null) {
		setTimeout(() => {
			frappe.call({
				method: "renewal_module.custom_module.page.crm_dashboard.target_data.get_sales_target_tq",
				args: { sales_person: sales_person },
				callback: (response) => {
	...
	get_calls_tm(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_calls_tm",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	get_appointments_tm(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_appointments_tm",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	get_demos_tm(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_demos_tm",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	get_leads_tm(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_leads_tm",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	get_sessions_tm(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_sessions_tm",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	get_trainings_tm(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_trainings_tm",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	get_fixed_amount(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_fixed_amount",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	get_variable_amount(sales_person = null) {
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.activity_target.get_variable_amount",
			args: { sales_person: sales_person },
			callback: (response) => {
				if (response.message) {
	...
	loadData(timeFilter = null, sales_person = null) {
		...
		const toDate = document.getElementById("to-date").value;
		frappe.call({
			method: "renewal_module.custom_module.page.crm_dashboard.sales_stage.get_filtered_opportunities",
			args: {
		...
		<div class="tab-content">
			<div class="tab-pane fade show active" id="review-content" role="tabpanel" aria-labelledby="review-tab">
				...
				<div class="row mt-2" style="border-radius:10px;background-color:#F3F3F3">
					...
					<div class="col-12 col-md-12 col-lg-6 mb-1">
						<div class="row mt-1">
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Opp Count(Lw)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Creation&forecast=Include&user=Administrator" target="_blank">Opp Count(Lw)
													</a>
												</span>
											</div>
										</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Closed Won(Lw)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&forecast=Include&user=Administrator" target="_blank">Closed Won (Lw)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Closed Lost(Lw)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Lost%22%5D&forecast=Include&user=Administrator" target="_blank">Closed Lost(Lw)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Renewal Count(LW)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&forecast=Include&opportunity_type=Renewal&user=Administrator" target="_blank">Renewal Count(LW)</a>
												</span>
											</div>
										</div>
									</div>
							...
							<div class="col-6 col-md-2 col-lg-4 mb-1">
								<div class="widget number-widget-box" data-widget-name="Profit Amount(LW)">
									<div class="widget-head">
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Profit Amount(LW)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&forecast=Include&user=Administrator" target="_blank">Profit Amount(LW)</a>
												</span>
											</div>
										</div>
									</div>
						...
						<div class="card1 card1-flush w-100 h-100 lg-100">
							<div class="card1-header">
								<div class="widget-title d-flex1 flex-column1">
									<span class="ellipsis pt-1 fs-6"><a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Brand&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&user=Administrator" target="_blank" class="title2">Brand Wise Sales (Lw)</a></span>
								</div>
							</div>
				...
				<div class="row mt-2" style="border-radius:10px;background-color:#F3F3F3">
					...
					<div class="col-12 col-md-12 col-lg-6 mb-1">
						<div class="row mt-1">
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Opp Count(Tm)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Creation&forecast=Include&user=Administrator" target="_blank">Opp Count(Tm)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Closed Won(Tm)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&forecast=Include&user=Administrator" target="_blank">Closed Won (Tm)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Closed Lost(Tm)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Lost%22%5D&forecast=Include&user=Administrator" target="_blank">Closed Lost(Tm)</a>
												</span>
											</div>
										</div>
									</div>
							...
							<div class="col-6 col-md-2 col-lg-4 mb-1">
								<div class="widget number-widget-box" data-widget-name="Renewal Count(Tm)">
									<div class="widget-head">
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Renewal Count(Tm)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&forecast=Include&opportunity_type=Renewal&user=Administrator" target="_blank">Renewal Count(Tm)</a>
												</span>
											</div>
										</div>
									</div>
							...
							<div class="col-6 col-md-2 col-lg-4 mb-1">
								<div class="widget number-widget-box" data-widget-name="Renewal Amount(Tm)">
									<div class="widget-head">
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Renewal Amount(Tm)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&forecast=Include&opportunity_type=Renewal&user=Administrator" target="_blank">Renewal Amount(Tm)</a>
												</span>
											</div>
										</div>
							...
							<div class="col-6 col-md-2 col-lg-4 mb-1">
								<div class="widget number-widget-box" data-widget-name="Profit Amount(Tm)">
									<div class="widget-head">
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Profit Amount(Tm)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&forecast=Include&user=Administrator" target="_blank">Profit Amount(Tm)</a>
												</span>
											</div>
										</div>
									</div>
				...
				<div class="row mt-2" style="border-radius:10px;background-color:#F3F3F3">
					...
					<!--<div class="col-6 col-md-2 col-lg-2">
						<div class="row mb-2 ms-1 mt-1 me-1">
							<div class="widget number-widget-box" data-widget-name="Active Rnwls(Lw)">
								<div class="widget-head">
									<div class="widget-label">
										<div class="widget-title">
											<span class="ellipsis" title="Active Rnwls(Lw)">
												<a href="/app/sales-funnel">Active(Lw)</a>
											</span>
										</div>
									</div>
								</div>
								<div class="widget-body">
									<div class="widget-content">
										<div class="number green ellipsis" id="rnwls-active-count">₹0</div>
									</div>
								</div>
							</div>
						</div>	
					...
					<div class="col-6 col-md-2 col-lg-2">
						<div class="row mb-2 ms-1 mt-1 me-1">
							<div class="widget number-widget-box" data-widget-name="Rnwls Pending(Lw)">
								<div class="widget-head">
									<div class="widget-label">
										<div class="widget-title">
											<span class="ellipsis" title="Rnwls PendingLw)">
												<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Renewal%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&status=Active&based_on=End+Date&group_by=Renewals" target="_blank">Pending(Lw)</a>
											</span>
										</div>
									</div>
								</div>
									...
									<div class="widget-label">
										<div class="widget-title">
											<span class="ellipsis" title="Cofed Rnwls(Lw)">
												<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Renewal%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&status=Cofed&based_on=End+Date&group_by=Renewals" target="_blank">Cofed(Lw)</a>
											</span>
										</div>
									</div>
								</div>
								...
								<div class="widget-head">
									<div class="widget-label">
										<div class="widget-title">
											...
											<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Renewal%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&status=Lost&based_on=End+Date&group_by=Renewals" target="_blank">Lost(Lw)</a>
										</span>
										</div>
									</div>
								</div>
					...
					<div class="col-6 col-md-2 col-lg-2">
						<div class="row mb-2 ms-1 mt-1 me-1">
							<div class="widget number-widget-box" data-widget-name="Rnwls New Opp(Lw)">
								<div class="widget-head">
									<div class="widget-label">
										<div class="widget-title">
											<span class="ellipsis" title="Rnwls New Opp(Lw)">
												<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Renewal%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&status=New+Opp&based_on=End+Date&group_by=Renewals" target="_blank">New Opp(Lw)</a>
											</span>
										</div>
									</div>
								</div>
					...
					<div class="col-6 col-md-2 col-lg-2">
						<div class="row mb-2 ms-1 mt-1 me-1">
							<div class="widget number-widget-box" data-widget-name="Rnwls Renewed(Lw)">
								<div class="widget-head">
									<div class="widget-label">
										<div class="widget-title">
											<span class="ellipsis" title="Rnwls Renewed(Lw)">
												<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Renewal%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=last+week&from_date=2023-04-01&to_date=2024-03-31&status=Renewed&based_on=End+Date&group_by=Renewals" target="_blank">Renewed(Lw)</a>
											</span>
										</div>
									</div>
								</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Opp Count(Tw)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&forecast=Include&based_on=Expected+Date&user=Administrator" target="_blank">Opp Count(Tw)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Opp Amount(Tw)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+week&from_date=2023-04-01&to_date=2024-03-31&forecast=Include&group_by=Opportunity&based_on=Expected+Date&user=Administrator" target="_blank">Opp Amount(Tw)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Rnwl Count(Tw)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&forecast=Include&opportunity_type=Renewal&user=Administrator" target="_blank">Rnwl Count(Tw)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Rnwl Amount(Tw)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&forecast=Include&opportunity_type=Renewal&user=Administrator" target="_blank">
													Rnwl Amount(Tw)
													</a>
												</span>
											</div>
						...
						<div class="row mb-1">
							<div class="col-12 col-md-6 col-lg-6">
								<div class="card1 card1-flush w-100 h-100 lg-100">
									<div class="col-12 col-md-12 col-lg-12">
										<div class="card1-header">
											<div class="widget-title d-flex1 flex-column1">
												<span class="ellipsis pt-1 fs-6"><a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Brand&forecast=Include&based_on=Expected+Date&user=Administrator" target="_blank" class="title2">Brand Wise Sales (TW)</a></span>
											</div>
										</div>
										<div class="card1-body style="height:auto;">
							...
							<div class="col-12 col-md-6 col-lg-6">
								<div class="card1 card1-flush w-100 h-100 lg-100">
									<div class="card1-header">
										<div class="widget-title d-flex1 flex-column1">
											<span class="ellipsis pt-1 fs-6"><a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+week&from_date=2023-04-01&to_date=2024-03-31&group_by=Item+Group&forecast=Include&based_on=Expected+Date&user=Administrator" target="_blank" class="title2 ellipsis">Item Group Wise Sales (TW)</a></span>
										</div>
									</div>
									<div class="card1-body" style="height:auto;">
								...
								<div class="widget number-widget-box" data-widget-name="Opp Count(TM)">
									<div class="widget-head">
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Opp Count(TM)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&forecast=Include&user=Administrator" target="_blank">Opp Count(TM)</a>
												</span>
											</div>
										</div>
									</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Opp Amount(TM)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&forecast=Include&user=Administrator" target="_blank">
													Opp Amount(TM)
													</a>
												</span>
											</div>
										...
										<div class="widget-label">
											<div class="widget-title">
												<span class="ellipsis" title="Rnwl Count(TM)">
													<a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Opportunity&based_on=Expected+Date&forecast=Include&opportunity_type=Renewal&user=Administrator" target="_blank">
													Rnwl Count(TM)
													</a>
												</span>
											</div>
						...
						<div class="row">
							<div class="col-12 col-md-6 col-lg-6">
								<div class="card1 card1-flush w-100 h-100 lg-100">
									<div class="card1-header">
										<div class="widget-title d-flex1 flex-column1">
											<span class="ellipsis pt-1 fs-6"><a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Brand&based_on=Expected+Date&user=Administrator" target="_blank" class="title2">Brand Wise Sales (TM)</a></span>
										</div>
									</div>
									<div class="card1-body style="height:auto;">
							...
							<div class="col-12 col-md-6 col-lg-6">
								<div class="card1 card1-flush w-100 h-100 lg-100">
									<div class="card1-header">
										<div class="widget-title d-flex1 flex-column1">
											<span class="ellipsis pt-1 fs-6"><a class="sales-link" data-base-url="http://192.168.23.185:8000/app/query-report/Opportunity%20Report?company=64+Network+Security+Pvt+Ltd+-+TG&timespan=this+month&from_date=2023-04-01&to_date=2024-03-31&group_by=Item+Group&based_on=Expected+Date&sales_stage=%5B%22Closed+Won%22%5D&user=Administrator" target="_blank" class="title2">Item Group Wise Sales (TM)</a></span>
										</div>
									</div>
									<div class="card1-body" style="height:auto;">
				...
				<div class="row mt-2" style="border-radius:10px;background-color:#F3F3F3">
					...
					<div class="col-4 col-md-4 col-lg-2 mb-2">
						<div class="widget number-widget-box" data-widget-name="Achieved">
							<div class="widget-head">
								<div class="widget-label">
									<div class="widget-title">
