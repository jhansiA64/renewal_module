...
class OppListPage {
	...
	injectDetailMeta(record) {
		...
		const $companyLabel = this.$content.find("#olCompanyLabel");
		if (companyVal && companyVal !== "-") {
			$companyLabel.html(
				`<a href="/app/company/${encodeURIComponent(companyVal)}" target="_blank"
					style="color:#3b7ef8;text-decoration:none;border-bottom:1px dotted #3b7ef8;"
					title="Open ${this.escapeAttr(companyVal)}">Company <i class="ti ti-external-link" style="font-size:10px;"></i></a>`
			);
		} else {
	...
	injectDetailContacts(record) {
		...
		me._detailContacts = (Array.isArray(record.contact_list) ? record.contact_list : [])
			.map(c => ({
				...
				name:      c.user_name || "—",       // display (same value, since docname = full name)
				email:     c.email_id  || "",
				phone:     c.mobile_no || "",
				role:      c.designation || "",
				poc:       Number(c.poc) || 0,
		...
		// primary = the row whose poc === 1 (default to first if none)
		...
		// figure out which contact is primary (by index)
		const primaryRef = String(record.contact_person || "").trim();
		if (me._detailPrimaryIdx === undefined) {
		...
		const renderContacts = () => {
			...
			$list.html(me._detailContacts.map((c, i) => {
				...
				<div style="background:${isPrimary ? "rgba(59,126,248,0.05)" : "#f8fafc"};
					...
					<!-- PRIMARY pill (top-right, only when primary) -->
					...
					<!-- remove × (top-right, only when NOT primary so it doesn't clash with pill) -->
					...
					<!-- Set Primary button (bottom, only when NOT primary and has docname) -->
					${!isPrimary && c.docname ? `
		...
		$list.find(".ol-set-primary").off("click").on("click", function(e) {
			e.stopPropagation();
			const i = parseInt($(this).data("ci"));
			if (!Number.isFinite(i) || !me._detailContacts[i]) return;
			me._detailPrimaryIdx = i;
			// set poc=1 on chosen, 0 on the rest
			me._detailContacts.forEach((c, j) => { c.poc = (j === i) ? 1 : 0; });
			renderContacts();
			me._saveDetailContacts(record);   // reuse the existing save — it writes the whole table
		});
	...
	if (createType === "quote-list") {
		frappe._quoteFromOpportunity = oppId;       // existing custom-page hook
		...
		<div class="page-wrap">
			...
			style="min-width:220px;padding:6px 0;">
					...
					<div style="padding:12px 16px 12px 0;border-right:1px solid rgba(0,0,0,0.06);">
						...
						<span id="olCompanyLabel">Company</span>
					</div>
