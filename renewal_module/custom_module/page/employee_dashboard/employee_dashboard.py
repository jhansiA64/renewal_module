...
def get_calls_count(sales_person=None):
    ...
    condition = ""
    if sales_person:
        condition = f"AND st.sales_person = %s"

    query = f"""
        SELECT COUNT(DISTINCT `tabCall List`.name) AS total 
        FROM `tabCall List`
        left join `tabSales Team` as st on st.parent= `tabCall List`.name
        WHERE `tabCall List`.creation BETWEEN %s AND %s {condition}
    """
