...
@frappe.whitelist()
def search_active_renewal_presets(customer_name="", filters=None, start=0, page_len=20):
    filters = frappe.parse_json(filters) if isinstance(filters, str) else (filters or {})
    ...
    brand = cstr(filters.get("brand")).strip()
    product = cstr(filters.get("product")).strip()
    end_date = cstr(filters.get("end_date")).strip()
    renewal_id = cstr(filters.get("renewal_id")).strip()

    conditions = [
        "rl.docstatus < 2",
        "ifnull(rl.status, '') = 'Active'",
        "ifnull(ri.item_code, '') != ''",
    ]
    values = {
        "start": start,
    ...
    if customer_name:
        conditions.append("ifnull(rl.customer_name, '') = %(customer_name)s")
        values["customer_name"] = customer_name

    if renewal_id:
        conditions.append("rl.name like %(renewal_id)s")
        values["renewal_id"] = f"%{renewal_id}%"

    if brand:
        conditions.append("ifnull(ri.brand, '') like %(brand)s")
        values["brand"] = f"%{brand}%"

    if product:
        conditions.append(
            "(" 
            "ifnull(rl.product_name, '') like %(product)s "
            "or ifnull(ri.item_name, '') like %(product)s "
            "or ifnull(ri.item_code, '') like %(product)s"
            ")"
        )
        values["product"] = f"%{product}%"

    if end_date:
        conditions.append("date(rl.end_date) = %(end_date)s")
        values["end_date"] = end_date

    return frappe.db.sql(
        f"""
        select
            rl.name as renewal_id,
            ifnull(rl.customer_name, '') as customer_name,
            ifnull(rl.product_name, '') as product_name,
            rl.end_date as end_date,
            ifnull(rl.status, '') as status,
            ifnull(ri.name, '') as item_row,
            ifnull(ri.item_code, '') as item_code,
            ifnull(ri.item_name, '') as item_name,
            ifnull(ri.brand, '') as brand,
            ifnull(ri.qty, 0) as qty,
            ifnull(ri.rate, 0) as rate,
            ifnull(ri.amount, 0) as amount,
            ifnull(ri.description, '') as description
        from `tabRenewal List` rl
        inner join `tabRenewal Item` ri on ri.parent = rl.name
        where {' and '.join(conditions)}
        order by rl.end_date asc, rl.modified desc, ri.idx asc
        limit %(start)s, %(page_len)s
        """,
        values,
        as_dict=True,
...
def get_opportunity_analytics(opportunity=None):
    ...
    # Filter items to only include those that should be in forecast
    # Exclude items where forecast is explicitly set to "Exclude"
    items = [
        item for item in items
        if (item.get("forecast") or item.get("custom_forecast") or "Include").strip().lower() != "exclude"
    ]
    ...
    try:
        ...
        if sq_names:
            ...
            scoped_rows = recommended_rows or []
...
def get_list_data(start=0, page_length=20, status="", probability="", owners=None, filters="[]"):
    ...
    # Optional fields
    title_field = first_existing_field(["title", "opportunity_title", "subject"])
    if title_field:                      select_fields.append(title_field)
...
def get_permitted_call_list(opportunity_name=None):
    ...
    docs = frappe.get_all(
        "Call List",
        filters={"reference":"Opportunity", "reference_to": opportunity_name},
        fields=[
