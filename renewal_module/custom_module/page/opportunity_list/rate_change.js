...
class opportunitydatapage {
	...
	openNewOppItemWizardInline(presetOpportunityType = null, editItemData = null) {
		...
		const syncInlineFormData = () => {
			...
			if (inlineDescriptionControl && typeof inlineDescriptionControl.get_value === "function") {
				itemData.description = String(inlineDescriptionControl.get_value() || "").trim();
			} else {
				const $desc = $root.find("#inline-wizard-description");
				itemData.description = String($desc.val?.() || $desc.text() || "").trim();
			}
		...
		const ensureInlineDescriptionEditor = () => {
			const $descHost = $body.find("#inline-wizard-description");
			if (!$descHost.length || !frappe?.ui?.form?.make_control) {
				inlineDescriptionControl = null;
				return;
			}

			$descHost.empty();
			inlineDescriptionControl = frappe.ui.form.make_control({
				parent: $descHost,
				df: {
					fieldtype: "Text Editor",
					fieldname: "inline_wizard_description",
					label: "",
					onchange: () => {
						syncInlineFormData();
			...
			inlineDescriptionControl.set_value(String(itemData.description || "").trim());

			const $input = inlineDescriptionControl.$input || inlineDescriptionControl.$wrapper?.find("textarea");
			if ($input?.length) {
				$input.css({ minHeight: "120px", resize: "vertical" });
			}
		};
		...
		const refreshInlineItemMetaFields = () => {
			...
			if (inlineDescriptionControl && typeof inlineDescriptionControl.set_value === "function") {
				inlineDescriptionControl.set_value(String(itemData.description || "").trim());
			}
		...
		const renderInlineWizard = () => {
			...
			$body.html(`
				...
				<div class="${panel4Class}" data-inline-panel="4">
					...
					<div class="new-opp-row one-col"><div class="new-opp-field"><label>Description</label><div id="inline-wizard-description" class="frappe-control"></div></div></div>
				</div>
  ...
  `);
	...
	/*
		...
		async saveOpportunityItemInlineEdit($row) {
			...
			const qtyInput = String($row.find(".item-edit-qty").val() || "").trim();
			const qtyResult = toIntegerQty(qtyInput);
			const qty = qtyResult.valid ? qtyResult.value : 0;
			...
			const margin = amount - spq_amount;
  ...
  `);
	...
	async get_connections_summary(name) {
		...
		const defs = [
			...
			},
			{
				...
				value: customerName,
				fields: ["customer", "party_name", "customer_name"],
			},
			{
				key: "orc_list",
				label: "ORC List",
			...
			},
			{
				...
				doctype: "Call List",
				value: opportunityName,
				fields: ["custom_opportunity", "opportunity"],
			},
		];
	...
	create_connection_doc(entry = {}, opportunityName = "") {
		...
		if (key === "call_list") {
			frappe.new_doc("Call List", { custom_opportunity: name });
			return;
		}
		if (key === "renewal_list") {
