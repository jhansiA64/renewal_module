...
// //   // Always render inside the Desk content pane
// //   const parent = $(wrapper).find('.layout-main-section');

// //   if (!wrapper.__oppui) {
// //     wrapper.__oppui = new custom.OpportunityUI(parent);
// //   }
// // };
// frappe.pages['opportunity-list'].on_page_load = function (wrapper) {
//   localStorage.removeItem('tickets_page_length');
// 	new OpportunityUI(wrapper);
// };



// frappe.pages['opportunity-list'].on_page_load = function (wrapper) {
//     new opportunitypage(wrapper);
// };
...
//             if (!frappe.opportunity_list_page || frappe.opportunity_list_page.wrapper !== pageWrapper) {
//                 frappe.opportunity_list_page = new opportunitypage(pageWrapper);
//             }
//             frappe.opportunity_list_page.render();
//         });
//     });
// };
...
//       <div class="row g-3">
//         <!-- Section 1: Opportunity List -->
//         <div class="col-lg-3 col-md-4">
//           <div class="card h-100">
//             <div class="card-body">
//               <h5 class="mb-3">Opportunity List</h5>
//               <input id="opp-search" class="form-control form-control-sm mb-2"
//                      placeholder="Search ID / Subject / Owner">
//               <div class="table-responsive" style="max-height:100vh;overflow:auto;">
//                 <table class="table table-sm table-hover" id="opp-table">
...
//               <div id="opp-details" class="small text-muted">Select an opportunity from the left to view details.</div>
//             </div>
//           </div>
...
//               <ul id="activity-summary" class="list-group list-group-flush"></ul>
//             </div>
//           </div>
//         </div>

//         <!-- Section 3: Activity Creation -->
//         <div class="col-lg-3">
//           <div class="card h-100">
//             <div class="card-body">
//               <h5 class="mb-3">Activity Creation</h5>
//               <ul class="nav nav-tabs mb-3" id="activity-tabs" role="tablist">
//                 <li class="nav-item"><a class="nav-link active" data-type="call"  href="#">Create Call</a></li>
//                 <li class="nav-item"><a class="nav-link"        data-type="event" href="#">Create Appointment</a></li>
//                 <li class="nav-item"><a class="nav-link"        data-type="task"  href="#">Create Task</a></li>
//                 <li class="nav-item"><a class="nav-link"        data-type="email" href="#">Log Email</a></li>
//               </ul>
...
//       </td></tr>
//     `);
//     // Reset middle/right columns
//     this.setLayout("list-only");
//     this.$details.html(`<span class="text-muted">${__("Select an opportunity from the left to view details.")}</span>`);
//     this.$items.empty();
//     this.$brandPie.empty();
//     this.$profitPie.empty();
//     this.$activitySummary.empty();
//     this.currentOpp = null;
//     this._activity_cache = [];
//     // Also reset the select-all header state
...
//     const stage = frappe.utils.escape_html(row.sales_stage || row.stage || row.status || "");
//     const close_date = frappe.format(row.expected_closing, { fieldtype: "Date" });
//     const owner = frappe.utils.escape_html(row.owner_full_name || row.owner || "");
//     const contact_name = frappe.utils.escape_html(row.contact_person_name || row.contact_person || "");
//     const contact_email = frappe.utils.escape_html(row.contact_email || "");
//     const avatar_txt = this.initials(contact_name || owner || title);

//     // ❗ No `${checked}` in template anymore
//     const $tr = $(`
//       <tr class="opp-row" data-name="${id}" style="cursor:pointer">
//         <td>
//           <input type="checkbox" class="opp-row-chk" data-name="${id}" aria-label="Select row ${id}"/>
...
//             <div class="avatar">${avatar_txt}</div>
//             <div>
//               <div class="name truncate-1">${contact_name || owner}</div>
//               <div class="muted truncate-1">${contact_email}</div>
//             </div>
//           </div>
//         </td>
//       </tr>
...
//     // Set checked state programmatically using the persistent selection set
//     if (this.selected && this.selected.has(id)) {
//       $tr.find('input.opp-row-chk').prop('checked', true);
//     }
...
//   // Sync header select-all checkbox with current page slice
//   if (typeof this.updateSelectAllState === "function") {
//     this.updateSelectAllState();
//   }
// }
 ...
 render_details(d) {
  ...
  //   if (!items.length) {
  //     this.$items.append(`
  //       <tr><td colspan="5" class="text-muted small">${__("No line items")}</td></tr>
  //     `);
  //     return;
  //   }
  //   items.forEach((it) => {
  ...
  //         <td>${frappe.format(it.qty, { fieldtype: "Float" })}</td>
  //         <td>${frappe.format(it.rate, { fieldtype: "Currency" })}</td>
  //         <td>${frappe.format(it.amount, { fieldtype: "Currency" })}</td>
  //         <td>${frappe.format(it.sales_stage, { fieldtype: "Data" })}</td>
  //         <td>${frappe.format(it.opportunity_type, { fieldtype: "Data" })}</td>
