...
function getOpportunityTitle(doc = {}) {
	const title = String(
		doc?.[OPP_CFG?.TITLE_FIELD || "title"]
		|| doc?.title
...
class opportunitydatapage {
	...
	bindNewOpportunityForm() {
		...
		const renumberNewOppItemRows = () => {
			$scope
				.find("#new-opp-items-table-body tr.new-opp-item-main-row")
				.each(function (idx) {
					const $row = $(this);
					$row.attr("draggable", true).attr("title", "Drag to reorder");
					$row.find(".new-opp-display-idx").text(fmtInt(idx + 1));
				});
		};

		const bindNewOppItemsDragAndDrop = () => {
			const $tbody = $scope.find("#new-opp-items-table-body");
			...
			$tbody
				.off("dragstart.new-opp-item-sort dragover.new-opp-item-sort drop.new-opp-item-sort dragend.new-opp-item-sort")
				.on("dragstart.new-opp-item-sort", "tr.new-opp-item-main-row", function (e) {
					$draggedRow = $(this);
					hasMoved = false;
					$draggedRow.addClass("dragging");
					e.originalEvent?.dataTransfer?.setData("text/plain", "new-opp-item-row");
					e.originalEvent.dataTransfer.effectAllowed = "move";
				})
				.on("dragover.new-opp-item-sort", "tr.new-opp-item-main-row", function (e) {
					e.preventDefault();
					const $target = $(this);
					if (!$draggedRow || !$target.length || $target.is($draggedRow)) return;
				...
				.on("drop.new-opp-item-sort", "tr.new-opp-item-main-row", function (e) {
					e.preventDefault();
				})
				.on("dragend.new-opp-item-sort", "tr.new-opp-item-main-row", function () {
		...
		const addItemRow = async (row = {}) => {
			//const nextRowIdx = $scope.find("#new-opp-items-table-body tr.new-opp-item-main-row").length + 1;
			...
			const tr = `
				<tr class="new-opp-item-main-row" draggable="true">
					<td class="line-items-center idx-col"><span class="new-opp-display-idx">${fmtInt(nextRowIdx)}</span></td>
					<td>
					...
					<td class="brand-col"><span class="cell-ellipsis" title="${frappe.utils.escape_html(brand || "-")}">${frappe.utils.escape_html(brand || "-")}</span></td>
					...
					<td class="date-col"><span class="cell-date" title="${frappe.utils.escape_html(expectedDate ? frappe.datetime.str_to_user(expectedDate) : "-")}">${frappe.utils.escape_html(expectedDate ? frappe.datetime.str_to_user(expectedDate) : "-")}</span></td>
			...
			//this.enableNewOppItemRowDragDrop();
		...
		$scope
			...
			.on("new-opp-items-visibility-refresh", function () {
				...
				bindNewOppItemsDragAndDrop();
				refreshNewOppItemsVisibility();
				//me.enableNewOppItemRowDragDrop();
			});
		renumberNewOppItemRows();
		bindNewOppItemsDragAndDrop();
		$scope
		...
		$scope
			...
			.on("click", "#new-opp-save", async function (e) {
				...
				if (itemRows.length) {
					const computedOpportunityAmount = me.getOpportunityAmountFromItems(itemRows);
					if (hasField(OPP_CFG.AMOUNT_FIELD)) {
						doc[OPP_CFG.AMOUNT_FIELD] = computedOpportunityAmount;
					}
					if (hasField("opportunity_amount")) {
						doc.opportunity_amount = computedOpportunityAmount;
					}
					if (hasField("base_opportunity_amount")) {
						doc.base_opportunity_amount = computedOpportunityAmount;
					}
				}
	...
	openNewOppItemWizardInline(presetOpportunityType = null, editItemData = null) {
		...
		let itemData = {
			...
			lost_reason: editItemData?.lost_reason || "",
		};
		...
		const checkClosedLostStage = (stageVal) => {
			if (stageVal === "Closed Lost") {
				let d = new frappe.ui.Dialog({
					...
					primary_action(values) {
						d.hide();
						itemData.lost_reason = values.lost_reason || "";
					}
				});
				d.on_cancel = () => {
					const prevStage = itemData.sales_stage || "Initial Analysis";
					$body.find("#inline-wizard-sales-stage").val(prevStage);
					syncInlineFormData();
		...
		const getSubmitPayload = () => {
			...
			const payload = {
				...
				lost_reason: String(itemData.lost_reason || "").trim(),
			};
		...
		const renderInlineWizard = () => {
			...
			$body
				...
				.on("change.newOppInlineWizard", "#inline-wizard-forecast, #inline-wizard-sales-stage, #inline-wizard-orc, #inline-wizard-commission-type, #inline-wizard-opportunity-type", function () {
					if (this.id === "inline-wizard-sales-stage") {
						checkClosedLostStage($(this).val());
					}
					syncInlineFormData();
	...
	async updateItemFromWizard(itemData) {
		...
		if (isDetailEdit) {
			...
			this.syncParentOpportunityAmountFromItems(latestDoc);

			if (normalizedItem.sales_stage === "Closed Lost") {
				latestDoc.opp_lost_reason = normalizedItem.lost_reason || latestDoc.opp_lost_reason || "";
			}

			// Sync custom status of the Opportunity
			const closedStages = ["Closed Won", "Closed Lost", "Dead"];
			let closedItemsFound = false;
			let openItemsFound = false;

			(latestDoc.items || []).forEach((item) => {
				if (closedStages.includes(item.sales_stage)) {
					closedItemsFound = true;
				} else {
					openItemsFound = true;
				}
			});

			if (closedItemsFound && openItemsFound) {
				latestDoc.custom_status = "Partially Closed";
			} else if (closedItemsFound && !openItemsFound) {
				latestDoc.custom_status = "Closed";
			} else {
				latestDoc.custom_status = "Open";
			}
	...
	bindLineItemsFullWidth() {
		...
		if (window.ResizeObserver) {
			const detailContent = $(this.page.wrapper)
				.find(".opportunity-list-details .detail-content")
				.get(0);
	...
	getHighestSalesStageFromItems(items = [], fallbackStage = "Initial Analysis") {
		...
		return bestStage || fallback;
	}

	getOpportunityAmountFromItems(items = []) {
		const toNumber = (value) => {
		...
		let includeTotal = 0;
		let hasClosedWonInclude = false;

		(items || []).forEach((item) => {
			const forecast = String(item?.forecast || item?.custom_forecast || "").trim().toLowerCase();
			if (forecast !== "include") return;

			const stage = String(item?.sales_stage || item?.status || "").trim().toLowerCase();
			...
			if (stage === "closed won") {
				hasClosedWonInclude = true;
				closedWonTotal += amount;
			} else {
				includeTotal += amount;
			}
		});

		return hasClosedWonInclude ? closedWonTotal : includeTotal;
	}

	syncParentOpportunityAmountFromItems(doc = {}) {
		if (!doc || typeof doc !== "object") return;
		const total = this.getOpportunityAmountFromItems(doc.items || []);

		const primaryField = OPP_CFG.AMOUNT_FIELD || "opportunity_amount";
		if (primaryField && (Object.prototype.hasOwnProperty.call(doc, primaryField) || primaryField === "opportunity_amount")) {
			doc[primaryField] = total;
		}

		if (primaryField !== "opportunity_amount" && Object.prototype.hasOwnProperty.call(doc, "opportunity_amount")) {
			doc.opportunity_amount = total;
		}

		if (Object.prototype.hasOwnProperty.call(doc, "base_opportunity_amount")) {
			doc.base_opportunity_amount = total;
		}
	}
	...
	render_rows(useFiltered = false) {
      ...
      `);
		...
		visible.forEach(doc => {
			const name = doc.name;
			const title = getOpportunityTitle(doc) || name;
	...
	async load_doc_details(name) {
		...
		// Extract data
		const titleVal = getOpportunityTitle(d) || d.name || "";
	...
	async pickRenewalPresetItem(opportunityType = "Renewal", customerName = "") {
		...
		let renewalStatusField = "";
		try {
			await frappe.model.with_doctype(renewalDoctype);
			const renewalMeta = frappe.get_meta(renewalDoctype);
			renewalStatusField = ["status", "workflow_state", "custom_status"].find((fieldname) =>
				renewalMeta?.fields?.some((df) => df.fieldname === fieldname)
			) || "";
		} catch (e) {
			renewalStatusField = "";
					...
					fields: [
						{
							...
							get_query: () => {
								const filters = {};
								if (customerName && customerFilterField) {
									filters[customerFilterField] = customerName;
								}
								if (renewalStatusField) {
									filters[renewalStatusField] = "Active";
								}
		...
		return await new Promise((resolve) => {
			...
			const renderRows = (rows) => {
				currentRows = (Array.isArray(rows) ? rows : []).filter((row) => {
					const status = String(row?.status || "").trim().toLowerCase();
					return !status || status === "active";
				});

				const uniqueByRenewal = new Map();
				for (const row of currentRows) {
					const renewalId = String(row?.renewal_id || "").trim();
					if (!renewalId) continue;
					if (!uniqueByRenewal.has(renewalId)) {
						uniqueByRenewal.set(renewalId, row);
					}
				}
				currentRowsByRenewal = Array.from(uniqueByRenewal.values());

				if (selectedRow?.renewal_id) {
					selectedRow = currentRowsByRenewal.find((row) => row.renewal_id === selectedRow.renewal_id) || null;
				}

				if (!currentRowsByRenewal.length) {
					selectedRow = null;
					$tbody.html(`<tr><td colspan="5" class="renewal-preset-picker-empty">No active renewals found for the selected filters.</td></tr>`);
					updatePrimaryState();
					return;
				}

				$tbody.html(
					currentRowsByRenewal.map((row, idx) => {
						...
						const statusLabel = frappe.utils.escape_html(String(row.status || "Active").trim() || "Active");
						const renewalId = frappe.utils.escape_html(String(row.renewal_id || "-").trim() || "-");
						const isSelected = selectedRow?.renewal_id === row.renewal_id;
						return `
							<tr data-row-index="${idx}" class="${isSelected ? "is-selected" : ""}">
								...
								<td><span class="renewal-preset-status">${statusLabel}</span></td>
								<td>${renewalId}</td>
							</tr>
						`;
					}).join("")
			...
			$picker.on("click", "tbody tr[data-row-index]", function () {
				...
				selectedRow = currentRowsByRenewal[idx] || null;
				renderRows(currentRowsByRenewal);
			});
  ...
  `);
	...
	async renderOpportunityItems(doc) {
		...
		const itemsHtml = items.map((item, idx) => {
			...
			return `
				<tr data-opportunity-item-name="${frappe.utils.escape_html(oppItemName)}" data-row-name="${frappe.utils.escape_html(rowName)}" data-row-idx="${idx}" draggable="true">
				<td class="line-items-center idx-col"><span class="detail-display-idx">${fmtInt(idx + 1)}</span></td>
				<td>
				<div class="line-item-title">
				...
				<td class="date-col"><span class="cell-date" title="${frappe.utils.escape_html(closureDateText)}">${frappe.utils.escape_html(closureDateText)}</span></td>
				<td class="actions-col line-items-center">
		...
		this.renumberDetailItemRows();
		this.setDetailItemsOrderDirty(false);
		this.bindDetailItemsDragAndDrop();
		...
		//this.enableDetailItemRowDragDrop();
	...
	/**enableNewOppItemRowDragDrop() {
		const $tbody = $(this.page.wrapper).find("#new-opp-items-table-body");
		if (!$tbody.length) return;

		let draggingEl = null;

		const updateNewRowOrderIndices = () => {
			$tbody.find("tr.new-opp-item-main-row").each(function (idx) {
				$(this).attr("data-row-idx", idx);
				$(this).find(".new-opp-display-idx").text(fmtInt(idx + 1));
			});
		};

		$tbody.find("tr.new-opp-item-main-row").attr("draggable", true);

		$tbody
			.off("dragstart.newOppItems dragend.newOppItems dragover.newOppItems drop.newOppItems")
			.on("dragstart.newOppItems", "tr.new-opp-item-main-row", function (e) {
			...
			.on("dragover.newOppItems", "tr.new-opp-item-main-row", function (e) {
				e.preventDefault();
				if (!draggingEl || this === draggingEl) return;
			...
			.on("drop.newOppItems", "tr.new-opp-item-main-row", function (e) {
				e.preventDefault();
			})
			.on("dragend.newOppItems", "tr.new-opp-item-main-row", function () {
				$(this).removeClass("is-dragging-row");
				draggingEl = null;
				updateNewRowOrderIndices();
			});
	}**/

	setDetailItemsOrderDirty(isDirty = false) {
		this._detailItemsOrderDirty = Boolean(isDirty);
		const $saveBtn = $("#detail-save-item-order");
		if (!$saveBtn.length) return;

		$saveBtn.toggleClass("d-none", !this._detailItemsOrderDirty);
	}

	renumberDetailItemRows() {
		const $tbody = $("#detail-items-tbody");
		if (!$tbody.length) return;

		$tbody.children("tr").not(".no-filter-results").each(function (idx) {
			const $row = $(this);
			$row.attr("data-row-idx", idx);
			$row.find(".item-row-edit-btn, .item-row-delete-btn").attr("data-row-idx", idx);
			$row.find(".detail-item-idx").text(fmtInt(idx + 1));
		});
	}

	bindDetailItemsDragAndDrop() {
		const me = this;
		const $tbody = $("#detail-items-tbody");
		if (!$tbody.length) return;
		...
		$tbody.children("tr").not(".no-filter-results")
			.attr("draggable", true)
			.attr("title", "Drag to reorder");

		$tbody
			.off("dragstart.detail-item-sort dragover.detail-item-sort drop.detail-item-sort dragend.detail-item-sort")
			.on("dragstart.detail-item-sort", "tr", function (e) {
				const $row = $(this);
				if ($row.hasClass("no-filter-results")) return;

				$draggedRow = $row;
				hasMoved = false;
				$row.addClass("dragging");
				e.originalEvent?.dataTransfer?.setData("text/plain", $row.attr("data-row-name") || "row");
				e.originalEvent.dataTransfer.effectAllowed = "move";
			})
			.on("dragover.detail-item-sort", "tr", function (e) {
				e.preventDefault();
				const $target = $(this);
				if (!$draggedRow || !$target.length || $target.is($draggedRow) || $target.hasClass("no-filter-results")) {
					return;
				}
			...
			.on("drop.detail-item-sort", "tr", function (e) {
				e.preventDefault();
			})
			.on("dragend.detail-item-sort", "tr", async function () {
				if (!$draggedRow) return;
				...
				me.renumberDetailItemRows();
				me.setDetailItemsOrderDirty(true);
			});
	}

	async persistDetailItemsOrder() {
		const docname = String(this._currentOpportunityName || "").trim();
		if (!docname) return;
		...
		const orderedRowNames = $tbody
			.children("tr")
			.not(".no-filter-results")
			.map(function () {
				return String($(this).attr("data-row-name") || "").trim();
			})
			.get()
			.filter(Boolean);

		if (orderedRowNames.length < 2) return;
		...
		const itemsByName = new Map(
			latestDoc.items.map((row) => [String(row?.name || "").trim(), row])
		);

		const reorderedItems = [];
		for (const rowName of orderedRowNames) {
			const row = itemsByName.get(rowName);
			if (row) reorderedItems.push(row);
		}

		if (!reorderedItems.length || reorderedItems.length !== latestDoc.items.length) {
			return;
		}

		latestDoc.items = reorderedItems.map((row, idx) => ({
			...row,
			idx: idx + 1,
		}));
		...
		try {
			await frappe.call({
				...
				args: { doc: latestDoc },
				freeze: true,
				freeze_message: __("Updating item order..."),
			});

			this._currentOpportunityDoc = latestDoc;
			this.setDetailItemsOrderDirty(false);
			frappe.show_alert({ message: __("Line item order updated"), indicator: "green" });
			await this.load_doc_details(docname);
		} catch (e) {
			frappe.msgprint(__("Unable to update line item order. Please try again."));
		}
	}

	/**async persistDetailItemRowOrder() {
		const docname = String(this._currentOpportunityName || "").trim();
		if (!docname) return;

		const $tbody = $(this.page.wrapper).find("#detail-items-tbody");
		if (!$tbody.length) return;

		const orderedRowNames = $tbody.find("tr")
			.map(function () {
				return String($(this).attr("data-row-name") || "").trim();
			})
			.get()
		...
		for (const row of latestDoc.items) {
			itemMap.set(String(row?.name || "").trim(), row);
		}

		const reordered = [];
		for (const rowName of orderedRowNames) {
			if (itemMap.has(rowName)) {
				reordered.push(itemMap.get(rowName));
				itemMap.delete(rowName);
			}
		}
		...
		const changed = reordered.length === latestDoc.items.length && reordered.some((row, idx) => row !== latestDoc.items[idx]);
		if (!changed) return;

		latestDoc.items = reordered.map((row, idx) => ({
			...row,
			idx: idx + 1,
		}));
		...
		frappe.show_alert({ message: __("Item order updated"), indicator: "green" });
		await this.load_doc_details(docname);
	}

	enableDetailItemRowDragDrop() {
		const $tbody = $(this.page.wrapper).find("#detail-items-tbody");
		if (!$tbody.length) return;
		...
		const updateDetailRowOrderIndices = () => {
			$tbody.find("tr").each(function (idx) {
				$(this).attr("data-row-idx", idx);
				$(this).find(".item-row-edit-btn, .item-row-delete-btn").attr("data-row-idx", idx).data("row-idx", idx);
				$(this).find(".detail-display-idx").text(fmtInt(idx + 1));
			});
		};
		...
		$tbody
			.off("dragstart.detailItems dragend.detailItems dragover.detailItems drop.detailItems")
			.on("dragstart.detailItems", "tr", function (e) {
				if ($(e.target).closest("button, a, input, textarea, select").length) {
					e.preventDefault();
					return;
				}
				...
				if (e.originalEvent?.dataTransfer) {
					e.originalEvent.dataTransfer.effectAllowed = "move";
					e.originalEvent.dataTransfer.setData("text/plain", "detail-item-row");
				}
			})
			.on("dragover.detailItems", "tr", function (e) {
				e.preventDefault();
				if (!draggingEl || this === draggingEl) return;
			...
			.on("drop.detailItems", "tr", function (e) {
				e.preventDefault();
			})
			.on("dragend.detailItems", "tr", async () => {
				$tbody.find("tr").removeClass("is-dragging-row");
				draggingEl = null;
				updateDetailRowOrderIndices();

				if (!orderChanged || isPersistingOrder) return;
				isPersistingOrder = true;
				try {
					await this.persistDetailItemRowOrder();
				} catch (e) {
					frappe.show_alert({ message: __("Unable to update item order."), indicator: "orange" }, 5);
				} finally {
	...
	setDetailOrcColumnsVisibility(showColumns = false) {
		// ORC columns are not displayed in the simplified table layout.
		return;
	}
	...
	bindOpportunityItemRowActions() {
		...
		$("#detail-save-item-order")
			.off("click")
			.on("click", async function (e) {
				e.preventDefault();
				e.stopPropagation();
				if (!me._detailItemsOrderDirty) return;
				await me.persistDetailItemsOrder();
			});
		...
		$tbody
			...
			.on("click", ".item-row-edit-btn", function (e) {
				...
				const editItemData = {
					...
					lost_reason: String(sourceDoc.opp_lost_reason || "").trim(),
					_editContext: "detail",
			...
			.on("click", ".item-row-delete-btn", function (e) {
				...
				frappe.confirm(
					...
					async () => {
						...
						me.syncParentSalesStageFromItems(latestDoc, latestDoc.sales_stage || "Initial Analysis");
						me.syncParentOpportunityAmountFromItems(latestDoc);
						frappe.call({
							method: "frappe.client.save",
							args: { doc: latestDoc },
							freeze: true,
  ...
  `);
		...
		$("#doc-actions-dropdown")
			...
			.on("click", "a[data-action]", (e) => {
				...
				if (action === "create_orc_list") {
					frappe.db.get_doc('Opportunity', name).then(doc => {
						frappe.new_doc("ORC List", { opportunity_id: name, customer_name: doc.party_name });
					});
					return;
				}

				if (action === "create_renewal_list") {
					frappe.db.get_doc('Opportunity', name).then(doc => {
						frappe.new_doc("Renewal List", {
							opportunity_id: name,
							customer: doc.customer || doc.party_name || "",
							customer_name: doc.party_name || doc.customer || "",
						});
					});
				}
			});
	...
	create_connection_doc(entry = {}, opportunityName = "") {
		...
		if (key === "orc_list") {
			frappe.new_doc("ORC List", { opportunity_id: name, customer_name: customerName });
			return;
		}
		...
		if (key === "renewal_list") {
			frappe.new_doc("Renewal List", { opportunity_id: name, customer_name: customerName });
			return;
		}
	...
	load_activity(name) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.get_opportunity_comments",
		...
		$(document)
			...
			.on("click", ".note-edit", function () {
				...
				const d = frappe.prompt(
					...
					function (values) {
						frappe.call({
							method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.update_opportunity_note",
		...
		$(document)
			...
			.on("click", ".note-delete", function (e) {
				...
				frappe.confirm(
					...
					() => {
						frappe.call({
							method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.delete_opportunity_note",
	...
	loadAndRenderNotes(opportunity_name) {
		...
		const self = this;
		frappe.call({
			method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.get_opportunity_notes",
  ...
  `);
	...
	bindcallcardsEvents(opportunityName) {
		...
		// ================= FETCH CALLS =================
		frappe.call({
			method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.get_permitted_call_list",
		...
		async function loadCallAvatars(list) {
			...
			const res = await frappe.call({
				method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.get_users_basic_info",
				args: {
		...
		async function getCustomerSalesPerson(customer) {
			...
			const res = await frappe.call({
				method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.get_customer_sales_person",
				args: { customer },
				silent: true
			});
		...
		async function openNewCallDialog(opportunity_name) {
			// Load default customer from the Opportunity
	...
	bindAppointmentCardsEvents(opportunityName) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.get_permitted_appointments",
			args: { opportunity_name },
			callback: async function (r) {
	...
	bindOpportunityActivitiesEvents(opportunityName) {
		...
		$('#add-comment-btn').off('click').on('click', async function () {
			...
			frappe.call({
				method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.add_custom_comment",
				args: {
					docname: opportunityName,
					content: content
				},
		...
		$('#save-quick-note-btn').off('click').on('click', function () {
			...
			frappe.call({
				method: "renewal_module.custom_module.page.opportunity_list.opportunity_list.add_opportunity_note",
				args: {
...
<div class="opportunity-list-details d-none">
  <style>
	...
	.detail-table th:nth-child(2){ width: 280px; }
	...
	.detail-table th:nth-child(2){ width: 220px; }
    ...
    .detail-table td.actions-col{
      ...
      max-width: 72px;
		...
		.detail-table th.idx-col,
		.detail-table td.idx-col{
			width: 40px;
			min-width: 40px;
			max-width: 40px;
			padding-left: 4px;
		...
		.detail-table th.date-col,
		.detail-table td.date-col{
			width: 120px;
			min-width: 120px;
			max-width: 120px;
		}

		.detail-table td.brand-col{
			text-align: center;
		}

		.detail-table td.brand-col .cell-ellipsis{
			font-size: 13px;
			text-align: center;
		}

		.detail-card--lineitems .detail-table thead th,
		.detail-card--lineitems .detail-table tbody td,
		#new-opp-items-table-wrap .detail-table thead th,
		#new-opp-items-table-wrap .detail-table tbody td{
			font-size: 13px;
		}

		.detail-table tbody td.date-col{
			overflow: visible;
		}

		.detail-table .cell-date{
			display: block;
			white-space: nowrap;
			overflow: visible;
			text-overflow: clip;
      ...
      <div class="ticket-actions">
        ...
        <div class="dropdown" id="doc-actions-dropdown">
          ...
          <ul class="dropdown-menu dropdown-menu-end">
			...
			<li><a class="dropdown-item" href="#" data-action="create_renewal_list">Renewal List</a></li>
          </ul>
        </div>
      </div>
    </div>
      ...
      <div class="detail-card detail-card--fullwidth detail-card--lineitems">
        ...
        <div class="table-wrap">
          <table class="detail-table">
            <thead>
              <tr>
				<th class="idx-col"><span class="th-ellipsis" title="Idx">Idx</span></th>
				...
				<th class="date-col"><span class="th-ellipsis" title="Closure Date">Closure Date</span></th>
                <th class="actions-col line-items-center"><span class="th-ellipsis" title="Edit">Edit</span></th>
              </tr>
            </thead>
            <tbody id="detail-items-tbody">
              <tr>
				<td colspan="10" class="muted">No line items</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="detail-add-row-wrap">
			<div class="dropdown" id="detail-add-item-actions">
				...
				<button type="button" class="btn btn-primary btn-sm d-none" id="detail-save-item-order">Save Order</button>
			</div>
        </div>
