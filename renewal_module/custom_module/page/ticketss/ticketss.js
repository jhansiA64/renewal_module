...
frappe.pages['ticketss'].on_page_load = function (wrapper) {
	localStorage.removeItem('tickets_page_length');
	new ticketspage(wrapper);
};

frappe.pages['ticketss'].on_page_show = function (wrapper) {
...
class ticketspage {
	...
	fetch_list_data({ reset = false, saved_filters = [], override_filters = {} } = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			try {
				...
				// Backend call
				frappe.call({
					method: "renewal_module.custom_module.page.ticketss.ticketss.get_list_data",
					args: {
	...
	render_rows(useFiltered = false) {
		...
		visible_tickets.forEach(ticket => {
			...
			tr.innerHTML = `
					...
					<td> <a href="/app/ticketss/${ticket.name}" class="ids link-reset issue-link" data-issue-name="${ticket.name}" title="${ticket.name} ">${ticket.name}</a></td>
					...
					<td class="ellipsis" title="${ticket.sla_resolution_by} ">
						${formatDate(ticket.sla_resolution_by)} 
					</td>
		...
		$(wrapper)
			...
			.on("click", ".issue-link", function (e) {
				...
				frappe.set_route("ticketss", issueName);
			});
		...
		async function renderAssignedAvatars(containerId, assignJson) {
			...
			const res = await frappe.call({
				method: "renewal_module.custom_module.page.ticketss.ticketss.get_users_basic_info",
				args: {
	...
	async loadFilters() {
		try {
			const res = await frappe.call({
				method: "renewal_module.custom_module.page.ticketss.ticketss.get_customer_filters"
			});
	...
	applyRoleBasedActionVisibility() {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.ticketss.ticketss.get_ticket_permissions",
			callback: (r) => {
				if (r.message) {
	...
	bindActionDropdownHandler() {
		...
		$actionsDropdown.off("click").on("click", async (e) => {
			...
			// ✅ Apply Assignment Rule
				...
				const calls = issues.map(name =>
					frappe.call({
						method: "renewal_module.custom_module.page.ticketss.ticketss.apply_assignment_rule",
						args: { doctype, name }
					})
				);
	...
	bindEvent(issue) {
		...
		$(this.wrapper).off("", "[data-action='set_working_agent']").on("click", "[data-action='set_working_agent']", async function (e) {
			...
			const users = await frappe.call({
				method: "renewal_module.custom_module.page.ticketss.ticketss.get_users_with_role",
				...
				fields: [
					{
						...
						get_query: () => {
							return {
								query: "renewal_module.custom_module.page.ticketss.ticketss.get_users_with_role_query"
							};
						}
					}
				],
		...
		$(this.wrapper).off("click", "#email-send").on("click", "#email-send", async function (e) {
			...
			const buildDefaultCc = async () => {
				try {
					const res = await frappe.call({
						method: "renewal_module.custom_module.page.ticketss.ticketss.get_cc_emails",
						args: { issue }
					});
			...
			try {
				const res = await frappe.call({
					method: "renewal_module.custom_module.page.ticketss.ticketss.get_contact_emails",
			...
			let email_dialog = new frappe.ui.Dialog({
				...
				primary_action: function (values) {
					...
					frappe.call({
						method: "renewal_module.custom_module.page.ticketss.ticketss.send_issue_email",
						args: {
					...
					const showMobileForm = async (type) => {
						...
						if (type === "Tasks") {
							...
							$panel.find("#mobile-ticket-panel-form-body").html(`
								...
								<div class="panel__field"><label>Expected End Date</label><input type="datetime-local" class="panel__input" id="mobile-task-end-date" /></div>
					...
					$modal.on("click.mobileTicketFormSave", "#mobile-ticket-panel-save-btn", async function () {
						...
						try {
							if (activeType === "Notes") {
								...
								await frappe.call({
									method: "renewal_module.custom_module.page.ticketss.ticketss.add_issue_note",
		...
		$(document)
			...
			.on("click", ".note-edit", function () {
				...
				const d = frappe.prompt(
					...
					function (values) {
						frappe.call({
							method: "renewal_module.custom_module.page.ticketss.ticketss.update_issue_note",
		...
		$(document)
			...
			.on("click", ".note-delete", function (e) {
				...
				frappe.confirm(
					...
					() => {
						frappe.call({
							method: "renewal_module.custom_module.page.ticketss.ticketss.delete_issue_note",
							args: { issue_name, idx },
							callback: function (r) {
								if (r.message?.notes) self.renderNotesList(r.message.notes);
							}
	...
	loadAndRenderNotes(issue_name) {
		...
		const self = this;
		frappe.call({
			method: "renewal_module.custom_module.page.ticketss.ticketss.get_issue_notes",
			args: { issue_name },
			callback: function (r) {
	...
	loadAndRenderUpcomingNonHeldCalls(issue_name) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.ticketss.ticketss.get_permitted_call_list",
			args: { issue_name },
			callback: function (r) {
	...
	loadAndRenderPendingTasks(issue_name) {
		...
		const esc = frappe.utils.escape_html;
		frappe.call({
			method: "renewal_module.custom_module.page.ticketss.ticketss.get_tasks_with_children",
			args: { issue_name },
			callback: function (r) {
	...
	loadAndRenderTodayOpenAppointments(issue_name) {
		...
		frappe.call({
			method: "renewal_module.custom_module.page.ticketss.ticketss.get_permitted_appointments",
			args: { issue_name },
			callback: function (r) {
					...
					return `
						<div class="border rounded bg-white p-2 mb-2">
							<div class="d-flex justify-content-between align-items-center gap-2">
								...
								<span class="badge bg-light text-dark border" title="${name}"><a href="/app/appointments/${name}" class="text-decoration-none fw-medium text-dark">${name}</a></span>
							</div>
							<div class="d-flex justify-content-between align-items-center gap-2">
	...
	bindcallcardsEvents(issueName) {
		...
		$(document)
			...
			.on("click", "#calls-card-load-more, #calls-card-title", function () {
				const advancedFilters = [
					["Call List", "reference", "=", "Issue"],
					["Call List", "reference_to", "=", issue_name],
				];
				const params = new URLSearchParams();
				params.set("filters", encodeURIComponent(JSON.stringify(advancedFilters)));
			...
			callback: async function (r) {
				...
				$(document)
					...
					.on("click", "#appointments-card-load-more, #appointments-card-title", function () {
						const advancedFilters = [
							["Appointment", "reference", "=", "Issue"],
							["Appointment", "reference_to", "=", issue_name],
						];
						const params = new URLSearchParams();
						params.set("filters", encodeURIComponent(JSON.stringify(advancedFilters)));
			...
			if (type === "Tasks") {
				...
				$("#ticket-panel-form-body").html(`
					<div class="ticket-form-grid">
						...
						<div class="row g-2">
							<div class="col-6 mb-2"><label class="form-label">Expected End Date</label><input type="datetime-local" id="ticket-task-end-date" class="form-control" /></div>
							<div class="col-6 mb-2"><label class="form-label">Priority</label><select id="ticket-task-priority" class="form-control"><option value="Low" selected>Low</option><option value="Medium">Medium</option><option value="High">High</option></select></div>
						</div>
		...
		$('#ticket-panel-save-btn').off('click').on('click', async function () {
			...
			try {
				if (activeType === 'Tasks') {
					...
					const priority = ($('#ticket-task-priority').val() || 'Low').trim();
	...
	bindResolutionEdit(issue_name) {
		...
		frappe.call({
			...
			args: {
				doctype: "Issue",
				filters: { name: issue_name },
				fieldname: ["resolution_details", "opening_date", "opening_time", "sla_resolution_date", "resolution_time", "user_resolution_time"]
			},
			callback: function (response) {
				const existing_resolution = response.message?.resolution_details || "";
		...
		this.approvalPollInterval = setInterval(() => {
			frappe.call({
				...
				callback: (r) => {
					...
					if (status === "approved") {
						this.stop_approval_poll_interval();
						// Keep resume manual: show Start button after approval, do not auto-start.
						this.set_manual_resume_required(issueName, true);
						frappe.show_alert({ message: __("Overtime approved. Click Start to continue."), indicator: "green" }, 5);
						this.load_issue_details(issueName, this.page);
		...
		const updateTimer = () => {
			frappe.call({
				...
				callback: (r) => {
					...
					if (!isWithinShift) {
						if (isApprovedServer) {
							// Continue timer outside shift when overtime is approved.
							shiftEndHandled = false;
							$timerLabel.text("Work:");
							$timerValue.text(this.format_work_time(countedSeconds));
		...
		$startBtn.off("click.worktimer").on("click.worktimer", () => {
			...
			frappe.call({
				...
				callback: (r) => {
					...
					const isAfterShiftApprovedRun = !!r?.message?.approved;
			...
			callback: function (r) {
				...
				$("#active_subscription-details").off("click").on("click", function () {
					...
					if (issue.active_renewals && issue.active_renewals.length) {
						...
						frappe.call({
							method: "renewal_module.custom_module.page.ticketss.ticketss.get_active_subscription_for_issue",
							args: { issue_name: issue.name },
							callback: function (res) {
				...
				$("#add-comment-btn").off("click").on("click", function () {
					...
					frappe.call({
						method: "renewal_module.custom_module.page.ticketss.ticketss.add_custom_comment",
						args: {
							docname: issueName,
							content: finalHtml
						},
				...
				function loadActivityTimeline(issueName) {
					frappe.call({
						method: "renewal_module.custom_module.page.ticketss.ticketss.get_issue_activity",
						args: { issue_name: issue.name },
						callback: function (res) {
				...
				async function loadAssignUsers(issue) {
					const res = await frappe.call({
						method: "renewal_module.custom_module.page.ticketss.ticketss.get_assignable_users",
						args: { issue_name: issue.name }
					});
			...
			frappe.call({
				...
				callback: (r) => {
					if (r.message) {
						...
						setTimeout(() => {
							//window.location.href = `/app/ticketss/${issue_name}`;
							frappe.set_route('ticketss', issue_name);
						}, 1000);
					...
					<div class="col-12">	
						<div class="ticket-meta mt-3">
							<div class="ticket-title d-flex align-items-center gap-2" style="min-width:0;">
								<span id="name" class="ticket-id" style="white-space:nowrap;"></span>
							...
							<div class="d-flex align-items-center flex-wrap gap-2 mt-2" style="row-gap:8px;">
								<button type="button" class="btn btn-default2 btn-primary btn-sm custom-start-btn d-none" id="custom-start-work-btn">Start</button>
								<div class="custom-work-timer d-none" style="background:#001f4d; color:white; padding:6px 10px; border-radius:5px; font-weight:bold; font-size:12px;">
									<span id="live-work-label">Work:</span>
									<span id="live-work-timer">00:00:00</span>
								</div>
								<button type="button" class="btn btn-default2 btn-danger btn-sm custom-stop-btn d-none" id="custom-stop-work-btn">Stop</button>
								<button type="button" class="btn btn-default2 btn-warning btn-sm overtime-request-btn d-none" id="overtime-request-btn">OT Request</button>
								<button type="button" class="btn btn-default2 btn-success btn-sm overtime-approve-btn d-none" id="overtime-approve-btn">Approve OT</button>
								<button type="button" class="btn btn-default2 btn-danger btn-sm overtime-reject-btn d-none" id="overtime-reject-btn">Reject OT</button>

								<div class="d-flex align-items-center gap-2 ms-auto" style="min-width:max-content;">
									<div id="ticket-details-assign-users"></div>
									<div><button id="time-logs-btn" class="btn btn-default2 btn-primary btn-sm">Time Logs</button></div>
