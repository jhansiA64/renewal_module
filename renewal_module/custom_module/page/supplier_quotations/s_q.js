          ...
          `;
			...
			$scope
				...
				.on("click", "#new-opp-save", async function (e) {
					...
					if (!hasField("supplier") && hasField("party_name")) doc.party_name = partyName;
		...
		async load_doc_details(name) {
			...
			if ($opportunity.length) {
				if (sourceOpportunity) {
					...
					const opportunityHref = `/app/opportunity-data/${encodeURIComponent(sourceOpportunity)}`;
					$opportunity
					...
					frappe.prompt(
						...
						async (values) => {
							...
							try {
								...
								if (!Object.keys(updateFields).length) updateFields.party_name = selectedSupplier;
			...
			const dialog = new frappe.ui.Dialog({
				...
				primary_action: async () => {
					...
					const pendingQuotationPayload = {
						...
						supplier: supplierFromItems || String(doc?.supplier || doc?.supplier_name || doc?.party_name || doc?.customer || "").trim(),
					...
					<div class="info-grid info-grid--2col">
						...
						<div class="info-col">
							<div class="info-row" style="margin-top:10px;">
								...
								<div class="value" id="detail-quotation-id">-</div>
							</div>
						</div>
					</div>
