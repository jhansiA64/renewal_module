...
(() => {
	...
	frappe.pages['supplier-quotations'].on_page_load = function (wrapper) {
		new supplierquotationdatapage(wrapper);
	};

	frappe.pages['supplier-quotations'].on_page_show = function (wrapper) {
		$('body').attr('data-route', 'supplier-quotations');   // ⭐ IMPORTANT FIX ⭐

		console.log("🔄 supplier-quotations page showing");
		...
		ensureSupportLayoutLoaded(() => {
			loadSupportLayout(pageWrapper, () => {
				if (!frappe.supplierquotationdata_page || frappe.supplierquotationdata_page.wrapper !== pageWrapper) {
					frappe.supplierquotationdata_page = new supplierquotationdatapage(pageWrapper);
				}
				frappe.supplierquotationdata_page.render();
			});
		});
	};
	...
	const OPP_CFG = {
		...
		API: {
			get_filters: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_filters",
			get_list_data: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_list_data",
			get_customer_options: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_customer_options",
			get_users_basic_info: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_users_basic_info",
			get_sales_person_details: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_sales_person_details",
			get_quotation_analytics: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_quotation_analytics",
			make_quotation_from_quotation: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.make_quotation_from_quotation"
		},

		// localStorage keys
		LS: {
			page_len: "supplier_quotation_list_page_length",
			last_filters: "supplier_quotation_list_last_filters"
		}
	};
	...
	// ---- PAGE CLASS ----
	class supplierquotationdatapage {
		constructor(wrapper) {
			this.wrapper = wrapper;
			this.page = frappe.ui.make_app_page({
				parent: wrapper,
				title: "Supplier Quotation List",
				single_column: true,
			});
		...
		render() {
			...
			const waitForContent = () => {
				...
				$content.empty().append(frappe.supplierquotationdata_page_template.body);
				this.handleRoute();
			};
			waitForContent();
		}
		...
		switchView(active) {
			...
			if (active === "list") {
				$scope.find(".quotation-list-list").removeClass("d-none").show();
				return;
			}
			if (active === "details") {
				$scope.find(".quotation-list-details").removeClass("d-none").show();
				return;
			}
			if (active === "new") {
				$scope.find(".quotation-list-new").removeClass("d-none").show();
			}
		}

		// -------- VIEWS --------
		show_list() {
			const view = window.renewal_module?.supplier_quotations?.views?.show_list;
			if (typeof view === "function") {
				return view(this, OPP_CFG, OPP_UI_VERSION);
			}
			console.warn("supplier_quotations_list.js is not loaded; using inline show_list logic");
			$(window).off("resize.oppLineItemsWidth");
			this.closeListFilterUi();
			this.switchView("list");

			$(this.page.wrapper)
				.off("click", "#new-quotation-btn")
				.on("click", "#new-quotation-btn", function (e) {
					e.preventDefault();
					frappe.set_route(OPP_CFG.ROUTE, "new");
				});

			setTimeout(async () => {
				this.setTitle("Supplier Quotations");
		...
		show_details(name) {
			const view = window.renewal_module?.supplier_quotations?.views?.show_details;
			if (typeof view === "function") {
				return view(this, name, OPP_CFG, OPP_UI_VERSION);
			}
			console.warn("supplier_quotations_detail.js is not loaded; using inline show_details logic");
			this.closeListFilterUi();
			this.switchView("details");
			this.setTitle(`supplier-quotations/${name}`);
		...
		async show_new() {
			const view = window.renewal_module?.supplier_quotations?.views?.show_new;
			if (typeof view === "function") {
				return view(this, OPP_CFG, OPP_UI_VERSION);
			}
			console.warn("supplier_quotations_form.js is not loaded; using inline show_new logic");
			$(window).off("resize.oppLineItemsWidth");
			this.closeListFilterUi();
			this.switchView("new");
			this.setTitle("New Supplier Quotation");
			console.log("Supplier Quotation List UI:", OPP_UI_VERSION);
			...
			const consumedFromOpportunity = await this.consumePendingQuotationFromOpportunity();
			if (!consumedFromOpportunity) {
				this.initNewOpportunityDefaults();
			}

			const urlParams = new URLSearchParams(window.location.search || "");
			const prefilledSupplier = (urlParams.get("supplier") || urlParams.get("customer") || "").trim();
			if (!consumedFromOpportunity && prefilledSupplier) {
				this.setNewOpportunityPartyName(prefilledSupplier);
			}
		...
		async consumePendingQuotationFromOpportunity() {
			...
			// Some route transitions can invoke show_new twice; preserve recent payload briefly.
			if (!payload) {
				const lastConsumedAt = Number(this._newOppLastConsumedAt || 0);
				if (this._newOppLastConsumedPayload && (Date.now() - lastConsumedAt) < 10000) {
					payload = this._newOppLastConsumedPayload;
				}
			}
			...
			this._newOppLastConsumedPayload = payload;
			this._newOppLastConsumedAt = Date.now();
			...
			const supplier = normalize(payload.supplier || payload.supplier_name || "");
			...
			const shippingAddress = normalize(
				payload.company_shipping_address_name
				|| payload.company_shipping_address
				|| ""
			);
			...
			const subject = normalize(payload.subject || payload.title || "");
			...
			const quotationId = normalize(payload.quotation_id || payload.quotation || payload.reference_quotation || "");
			...
			if (supplier) this.setNewOpportunityPartyName(supplier);
			...
			if (sourceOpportunity) this.setNewOpportunityOpportunityId(sourceOpportunity);
			if (quotationId) this.setNewOpportunityQuotationId(quotationId);
			if (subject) {
				$scope.find("#new-opp-subject").val(subject);
			}
			...
			if (typeof this._addNewOppItemRow === "function") {
				...
				items.forEach((row) => {
					...
					const rate = this.getItemSpqRateNumber(row, row?.rate, spqFieldCandidates?.rateFields || []);
					const amount = this.getItemSpqAmountNumber(row, row?.amount || (qty * rate), spqFieldCandidates?.amountFields || []) || (qty * rate);
					...
					this._addNewOppItemRow({
						...
						supplier: normalize(row?.supplier || row?.supplier_name || row?.custom_supplier),
						hsncode: normalize(row?.hsncode || row?.hsn_code || row?.gst_hsn_code),
						uom: normalize(row?.uom || row?.stock_uom),
						source_detail_name: normalize(row?.source_detail_name || row?.opportunity_item || row?.name),
						opportunity_item: normalize(row?.opportunity_item || row?.source_detail_name || row?.name),
			...
			setTimeout(async () => {
				...
				if (!this.getNewOpportunityShippingAddress() && currentCompany) {
					try {
						const companyDoc = await frappe.db.get_doc("Company", currentCompany);
						let fallbackShipping = normalize(
							companyDoc?.shipping_address_name
							|| companyDoc?.shipping_address
						);

						if (!fallbackShipping && canReadDoctype("Dynamic Link")) {
							...
							const billingAddress = normalize(this.getNewOpportunityCompanyAddress());
							fallbackShipping = normalize(
								(Array.isArray(linkedRows) ? linkedRows : [])
									.map((row) => row?.parent)
									.find((address) => normalize(address) && normalize(address) !== billingAddress)
								|| ""
							);
						}

						if (!fallbackShipping) {
							fallbackShipping = normalize(this.getNewOpportunityCompanyAddress());
						}
				...
				if (!this.getNewOpportunityCompanyAddress() && currentCompany) {
					try {
						...
						if (!fallbackCompanyAddress) {
							if (canReadDoctype("Dynamic Link")) {
								const linkedRows = await frappe.db.get_list("Dynamic Link", {
									fields: ["parent", "parenttype"],
									filters: {
										link_doctype: "Company",
						...
						if (fallbackCompanyAddress) {
							...
							if (!this.getNewOpportunityShippingAddress()) {
								this.setNewOpportunityShippingAddress(fallbackCompanyAddress);
							}
						}
					} catch (e) {
						// no-op
		...
		bindNewOpportunityForm() {
			...
			const getRenewalCustomerFilterField = async () => {
				...
				try {
					...
					const candidates = ["customer", "supplier", "customer_name"];
			...
			const getInlineItemDetailFields = ($row) => {
				...
				const preferredOrder = [
					"qty", "rate", "amount", "uom",
					"brand", "item_group", "hsncode", "description",
				];
			...
			const addItemRow = async (row = {}) => {
				...
				const recommended = Number(row.recommended_ || row.recommended || 0) ? "checked" : "";
				...
				<tr class="new-opp-item-main-row">
					<td>
						...
						<div class="d-none new-opp-item-hidden-fields">
							...
							<input type="checkbox" class="new-opp-item-recommended d-none" ${recommended}>
					...
					<td class="line-items-center"><input type="checkbox" class="new-opp-display-recommended" ${Number(row.recommended_ || row.recommended || 0) ? "checked" : ""} disabled></td>
			...
			`;
				...
				$row.data("currentItemValues", {
					...
					recommended_: Number(row.recommended_ || row.recommended || 0) ? 1 : 0,
			...
			const prefillSalesTeamFromSupplier = async () => {
				...
				const supplierName = String(me.getNewOpportunityPartyName() || "").trim();
				...
				try {
					const supplierDoc = await frappe.db.get_doc("Supplier", supplierName);
					if (requestId !== me._newOppSalesPrefillReq) return;
					if (String(me.getNewOpportunityPartyName() || "").trim() !== supplierName) return;

					const rows = Array.isArray(supplierDoc?.sales_team) ? supplierDoc.sales_team : [];
			...
			const prefillContactAndAddressFromSupplier = async () => {
				if (Date.now() < Number(me._newOppPreserveFromOpportunityUntil || 0)) {
					return;
				}
				...
				try {
					...
					let customerAddress = String(
						supplierDoc?.supplier_primary_address
						|| supplierDoc?.primary_address
					...
					const primaryContact = String(
						supplierDoc?.supplier_primary_contact
						|| supplierDoc?.primary_contact
						|| supplierDoc?.default_contact
						|| ""
					).trim();

					if (!customerAddress || !primaryContact) {
						const linkedRows = canReadDoctype("Dynamic Link")
							? await frappe.db.get_list("Dynamic Link", {
								fields: ["parent", "parenttype"],
								filters: {
						...
						if (!primaryContact && linkedContacts.length) {
							const firstContact = linkedContacts[0];
							if (firstContact) {
								me._newOppContactList = [firstContact];
								me._newOppSelectedContact = firstContact;
							}
						}
					}

					me.setNewOpportunityCustomerAddress(customerAddress);

					if (primaryContact) {
						me._newOppContactList = [primaryContact];
						me._newOppSelectedContact = primaryContact;
					}
			...
			const runPartyPrefill = () => {
				prefillSalesTeamFromSupplier();
				prefillContactAndAddressFromSupplier();
				prefillPaymentTermsFromSupplier();
			...
			const prefillCompanyAddressFromCompany = async () => {
				...
				try {
					...
					let shippingAddress = String(companyDoc?.shipping_address_name || companyDoc?.shipping_address || "").trim();
					...
					if ((!companyAddress || !shippingAddress) && canReadDoctype("Dynamic Link")) {
						const linkedRows = await frappe.db.get_list("Dynamic Link", {
							fields: ["parent", "parenttype"],
							filters: {
								link_doctype: "Company",
						...
						if (!companyAddress) {
							companyAddress = linkedAddresses[0] || "";
						}
						if (!shippingAddress) {
							shippingAddress = linkedAddresses.find((address) => address !== companyAddress) || linkedAddresses[1] || "";
						}
					}
					...
					me.setNewOpportunityShippingAddress(shippingAddress);
					syncAddressPreviews();
				} catch (e) {
					if (requestId !== me._newOppCompanyPrefillReq) return;
				}
			...
			const getSupplierTaxProfile = async () => {
				const supplierName = String(me.getNewOpportunityPartyName() || "").trim();
				if (!supplierName) return { gstCategory: "", taxCategory: "" };

				const cache = me._newOppCustomerTaxProfileCache || {};
				if (cache.customerName === supplierName) {
					return {
				...
				try {
					const supplierDoc = await frappe.db.get_doc("Supplier", supplierName);
					const profile = {
						gstCategory: String(supplierDoc?.gst_category || "").trim(),
						taxCategory: String(supplierDoc?.tax_category || "").trim(),
					};
			...
			const isSezLikeCustomer = async () => {
				const profile = await getSupplierTaxProfile();
			...
			const resolveTaxesTemplateName = async (wantInState = true) => {
				...
				const pickNonRcmTemplate = async (rows = [], strictDirection = true) => {
					...
					for (const candidate of candidates) {
						try {
							const candidateDoc = await frappe.db.get_doc("Purchase Taxes and Charges Template", candidate);
				...
				try {
					const rows = await frappe.db.get_list("Purchase Taxes and Charges Template", {
						fields: ["name"],
						filters: {
							...(company ? { company } : {}),
				...
				try {
					const fallbackRows = await frappe.db.get_list("Purchase Taxes and Charges Template", {
						fields: ["name"],
						filters: {
							...(company ? { company } : {}),
						},
			...
			const resolveSezTaxesTemplateName = async () => {
				...
				for (const candidate of candidates) {
					try {
						const doc = await frappe.db.get_doc("Purchase Taxes and Charges Template", candidate);
			...
			const syncTaxCategoryFromTemplate = async () => {
				...
				try {
					const templateDoc = await frappe.db.get_doc("Purchase Taxes and Charges Template", templateName);
			...
			const prefillPaymentTermsFromSupplier = async () => {
				if (Date.now() < Number(me._newOppPreserveFromOpportunityUntil || 0)) return;
				...
				try {
					...
					const template = String(
						supplierDoc?.payment_terms_template
						|| supplierDoc?.default_payment_terms_template
						|| supplierDoc?.payment_terms
						|| ""
					).trim();
			...
			me._runNewOppPaymentTermsPrefill = prefillPaymentTermsFromSupplier;

			const wireTaxControls = () => {
			...
			const computeTaxesPreviewData = async () => {
				...
				const findCompanyCompatibleTemplate = async ({ selectedCompany, categoryHint = "", requireTaxFree = false, requireNonRcm = false, requiredDirection = "" } = {}) => {
					...
					for (const candidateName of candidateNames) {
						try {
							const candidateDoc = await frappe.db.get_doc("Purchase Taxes and Charges Template", candidateName);
				...
				let templateDoc = await frappe.db.get_doc("Purchase Taxes and Charges Template", templateName);
				...
				if (
					...
					if (replacementName) {
						...
						templateDoc = await frappe.db.get_doc("Purchase Taxes and Charges Template", replacementName);
			...
			me._renderNewOppPaymentTermsPreview = null;
          ...
          `;
			...
			$scope
				...
				.on("click", ".new-opp-edit-row", async function (e) {
					...
					const editItemData = {
						...
						recommended_: Number(stored.recommended_ || stored.recommended || 0) ? 1 : 0,
			...
			$scope
				...
				.on("click", "#new-opp-next", function (e) {
					...
					if (current === 1) {
						...
						if (!partyName) {
							frappe.show_alert({ message: __("Please select a Supplier before proceeding."), indicator: "red" }, 4);
							return;
						}
						if (!transactionDate) {
			...
			$scope
				...
				.on("click", "#new-opp-save", async function (e) {
					...
					const customerAddress = me.getNewOpportunityCustomerAddress() || me.getNewOpportunityShippingAddress();
					...
					const opportunityFrom = "Supplier";
					const linkedOpportunityId = me.getNewOpportunityOpportunityId();
					const linkedQuotationId = me.getNewOpportunityQuotationId();
					const title = subject || [partyName || "Supplier", frappe.datetime.nowdate()]
						.filter(Boolean)
						.join(" - ");
					...
					const itemRows = $scope
						...
						.map((_, row) => {
							...
							const recommended_ = $row.find(".new-opp-item-recommended").length ? ($row.find(".new-opp-item-recommended").is(":checked") ? 1 : 0) : (Number(stored.recommended_ || stored.recommended || 0) ? 1 : 0);
					...
					if (hasField("supplier")) doc.supplier = partyName;
					if (hasField("supplier_name")) doc.supplier_name = partyName;
					if (!hasField("supplier") && hasField("supplier")) doc.supplier = partyName;
					...
					const buildQuotationTaxesFromTemplate = async (templateName) => {
						...
						const taxChildDoctype = getTableOptions("taxes") || "Purchase Taxes and Charges";
						...
						try {
							const templateDoc = await frappe.db.get_doc("Purchase Taxes and Charges Template", normalizedTemplate);
							const rows = Array.isArray(templateDoc?.taxes) ? templateDoc.taxes : [];
							return rows.map((row) => {
					...
					if (customerAddress) {
						setParentFieldIfExists(["supplier_address", "customer_address"], customerAddress);
					}
					if (shippingAddress && hasField("shipping_address_name")) doc.shipping_address_name = shippingAddress;
					if (shippingAddress && hasField("shipping_address")) doc.shipping_address = shippingAddress;
					...
					if (linkedQuotationId) {
						setParentFieldIfExists(["quotation_id", "quotation", "reference_quotation"], linkedQuotationId);
					}
					...
					if (itemRows.length && hasField("items")) {
						...
						doc.items = itemRows.map((row) => {
							...
							setIfField(child, ["recommended_", "recommended"], row.recommended_);
					...
					try {
						...
						frappe.msgprint(__("Failed to create Supplier Quotation. Please try again."));
		...
		openNewOppItemWizardInline(presetOpportunityType = null, editItemData = null) {
			...
			const syncInlineFormData = () => {
				...
				itemData.recommended_ = $root.find("#inline-wizard-recommended").is(":checked") ? 1 : 0;
			...
			const getSubmitPayload = () => {
				...
				const payload = {
					...
					supplier: String(itemData.supplier || "").trim(),
					recommended_: Number(itemData.recommended_ || 0) ? 1 : 0,
			...
			const renderInlineWizard = () => {
				...
				<div class="${panel2Class}" data-inline-panel="2">
					...
					<div class="new-opp-row four-col">
						...
						<div class="new-opp-field"><label>Recommended</label><div><input id="inline-wizard-recommended" type="checkbox" ${itemData.recommended_ ? "checked" : ""}> Mark Recommended</div></div>
			...
			`);
				...
				$body
					...
					.on("input.newOppInlineWizard", "#inline-wizard-supplier", function () {
						syncInlineFormData();
					})
		...
		async addItemFromWizard(itemData) {
			...
			const normalizedItem = {
				...
				description: String(itemData?.description || "").trim(),
				supplier: String(itemData?.supplier || itemData?.supplier_name || "").trim(),
				renewal_id: String(itemData?.renewal_id || "").trim(),
				commission_type: String(itemData?.commission_type || "").trim(),
				rate_value: Number(itemData?.rate_value || 0),
				orc: Number(itemData?.orc || 0) ? 1 : 0,
				recommended_: Number(itemData?.recommended_ || itemData?.recommended || 0) ? 1 : 0,
			};
		...
		async updateItemFromWizard(itemData) {
			...
			if (isDetailEdit) {
				...
				Object.assign(targetRow, {
					...
					supplier: String(itemData?.supplier || targetRow.supplier || targetRow.supplier_name || "").trim(),
					...
					recommended_: normalizedItem.recommended_,
			...
			$rowElement.find(".new-opp-display-recommended").prop("checked", normalizedItem.recommended_ ? true : false);
			...
			$rowElement.find(".new-opp-item-recommended").prop("checked", normalizedItem.recommended_ ? true : false);
		...
		ensureNewOpportunityControls() {
			...
			ensureLinkControl("#new-opp-party-name-control", "_newOppPartyControl", "Supplier");
			ensureLinkControl("#new-opp-company-control", "_newOppCompanyControl", "Company");
			ensureLinkControl("#new-opp-opportunity-id-control", "_newOppOpportunityIdControl", "Opportunity");
			ensureLinkControl("#new-opp-quotation-id-control", "_newOppQuotationIdControl", "Quotation");
			...
			ensureLinkControl("#new-opp-customer-address-control", "_newOppCustomerAddressControl", {
				options: "Address",
				get_query: () => {
					const supplierName = String(this.getNewOpportunityPartyName() || "").trim();
		...
		getNewOpportunityOpportunityId() {
			if (this._newOppOpportunityIdControl?.get_value) {
				return String(this._newOppOpportunityIdControl.get_value() || "").trim();
			}
			return "";
		}

		setNewOpportunityOpportunityId(value = "") {
			if (this._newOppOpportunityIdControl?.set_value) {
				this._newOppOpportunityIdControl.set_value(String(value || "").trim());
			}
		}

		getNewOpportunityQuotationId() {
			if (this._newOppQuotationIdControl?.get_value) {
				return String(this._newOppQuotationIdControl.get_value() || "").trim();
			}
			return "";
		}

		setNewOpportunityQuotationId(value = "") {
			if (this._newOppQuotationIdControl?.set_value) {
				this._newOppQuotationIdControl.set_value(String(value || "").trim());
			}
		}
		...
		setNewOpportunityCompanyAddress(value = "") {
			...
			if (this._newOppCompanyAddressControl?.set_value) {
				this._newOppCompanyAddressControl.set_value(normalizedValue);
			}
		}
		...
		initNewOpportunityDefaults() {
			...
			$scope.find("#new-opp-party-label").text("Supplier");
			this.setNewOpportunitySalesTeam([]);
			this.setNewOpportunityPartyName("");
			this.setNewOpportunityOpportunityId("");
			this.setNewOpportunityQuotationId("");
			...
			$scope.find("#new-opp-items-table-body").html("");
			$scope.find("#new-opp-save").prop("disabled", false).text("Create Supplier Quotation");
			$scope.trigger("new-opp-contact-changed");
			this.gotoNewWizardStep(1);
		}
		...
		attachInlineLinkPortal(control) {
			...
			// Try immediately; if the awesomplete <ul> isn't in the DOM yet
			// (e.g. control rendered inside a hidden panel), retry on first user interaction.
			if (!setupPortal()) {
				$input.off("focus.inlinePortalSetup input.inlinePortalSetup");
				const retrySetup = () => {
					if (setupPortal()) {
						$input.off("focus.inlinePortalSetup input.inlinePortalSetup");
					}
				};
				$input.on("focus.inlinePortalSetup input.inlinePortalSetup", retrySetup);
			}
		}
        ...
        <tr><td colspan="11" class="text-center text-muted py-4">No Supplier Quotation found.</td></tr>
      `);
			...
			visible.forEach(doc => {
				...
				const party = doc[OPP_CFG.PARTY_NAME_FIELD] || doc.supplier || doc.supplier_name || doc.customer || doc.lead || "";
        ...
        <td class="ellipsis" title="${validTill}">${this.formatDate(validTill)}</td>
    ...
    `;
		...
		async load_doc_details(name) {
			...
			const party = d[OPP_CFG.PARTY_NAME_FIELD] || d.supplier || d.supplier_name || d.customer || d.lead || "";
			...
			const companyDetails = d.company || d.company_name || d.organization || d[OPP_CFG.PARTY_NAME_FIELD] || d.supplier || d.supplier_name || d.customer || d.lead || "";
			const sourceOpportunity = String(d.opportunity || d.prevdoc_docname || "").trim();
			const quotationId = String(d.quotation_id || d.quotation || d.reference_quotation || "").trim();
			...
			if (party) {
				$("#detail-customer-name").html(
					`<a href="/app/supplier/${encodeURIComponent(party)}" 
						title="${party}">
						${party}
					</a>`
				);
			} else {
				$scope.find("#detail-customer-name").text("-");
			}
			...
			const $quotationId = $("#detail-quotation-id");
			if ($quotationId.length) {
				//$quotationId.text(quotationId || "-").attr("title", quotationId || "");
				if (quotationId) {
					const safeQuotationId = frappe.utils.escape_html(quotationId);
					const quotationHref = `/app/quotation-list/${encodeURIComponent(quotationId)}`;
					$quotationId
						.html(`<a class="link-reset" href="${quotationHref}" title="${safeQuotationId}">${safeQuotationId}</a>`)
						.attr("title", quotationId);
				} else {
					$quotationId.text("-").attr("title", "");
				}
			}
		...
		async extractRenewalItemPreset(renewalDoc, renewalId, opportunityType = "Renewal", selectedItemRowName = "") {
			...
			let rate = Number(itemRow.spq_rate || itemRow.custom_spq_rate || itemRow.rate || doc.spq_rate || doc.rate || 0) || 0;
			if (!rate) {
			...
			return {
				...
				supplier: String(itemRow.supplier || itemRow.supplier_name || itemRow.custom_supplier || "").trim(),
		...
		async renderOpportunityContactSection(doc) {
			...
			this._currentPartyName = doc[OPP_CFG.PARTY_NAME_FIELD] || doc.supplier || doc.supplier_name || doc.customer || doc.lead || "";
      ...
      `;
		...
		async renderOpportunitySalesTeamSection(doc) {
			...
			const ownerUser = String(doc.owner || doc.created_by || "").trim();
			if (!ownerUser) {
				this._currentSalesTeamSummary = [];
				$container.html(`<span class="text-muted">-</span>`);
				return;
			}

			const ownerMeta = frappe.boot?.user_info?.[ownerUser] || {};
			const ownerEmail = ownerUser.includes("@") ? ownerUser : String(ownerMeta.email || "").trim();
			const displayEmail = ownerEmail || ownerUser;

			this._currentSalesTeamSummary = [{
				sales_person: displayEmail,
				owner_user: ownerUser,
				email: displayEmail,
			}];

			const html = this._currentSalesTeamSummary
				.map((row) => {
					const email = frappe.utils.escape_html(row.email || "-");

					return `
        <div class="contact-entry gap-1 d-flex align-items-center" style="width:100%; margin-bottom:6px;">
          <div style="flex: 1; min-width: 0;">
            <span class="opp-sales-team-name text-truncate" style="cursor:pointer; display: block;" data-idx="0" title="${email}">
              ${email}
            </span>
          </div>
        </div>
      `;
		...
		async renderOpportunityAddressSection(doc) {
			...
			const pickDistinct = (values = [], used = new Set()) => {
				const normalizedUsed = new Set(Array.from(used).map((v) => String(v || "").trim()).filter(Boolean));
				const candidates = values.map((value) => String(value || "").trim()).filter(Boolean);
				const distinct = candidates.find((value) => !normalizedUsed.has(value));
			...
			const shippingCandidates = [
				...
				doc.company_shipping_address_name,
				doc.company_shipping_address,
			];
			const companyCandidates = [
				doc.company_address,
				doc.company_address_name,
				doc.default_company_address,
			];

			const customerAddress = pickFirst(customerCandidates);
			const shippingAddress = pickDistinct(shippingCandidates, new Set([customerAddress]));
			const companyAddress = pickDistinct(companyCandidates, new Set([customerAddress, shippingAddress]));
						...
						fields: [
							{
								...
								get_query: () => {
									...
									const customerName = doc.customer
										|| (opportunityFrom === "customer" ? (doc.supplier || doc[OPP_CFG.PARTY_NAME_FIELD] || "") : "");
		...
		bindOpportunitySalesTeamActions() {
			...
			$("#detail-sales-team")
				...
				.on("click", ".opp-sales-team-name", async function () {
					...
					if (row.owner_user) {
						me.showRightPanelPersonCard({
							title: row.sales_person,
							role: "Created By",
							email: row.email || (String(row.owner_user || "").includes("@") ? row.owner_user : ""),
							mobile: "",
							metaLabel: "User",
							metaValue: row.owner_user || "-",
						});
						return;
					}
  ...
  `);
		...
		async renderOpportunityItems(doc) {
			...
			if (!items.length) {
				$itemsTable.html(`<tr><td colspan="5" class="muted">No line items</td></tr>`);
    ...
    `;
		...
		setDetailOrcColumnsVisibility(showColumns = false) {
			// ORC/commission/rate-value columns were removed from detail items table.
			// Keep this method as a no-op because existing flows still call it.
			return;
		}
    ...
    `);
		...
		bind_doc_actions(name) {
			...
			$(this.page.wrapper)
						...
						[
							{
								...
								options: "Supplier",
								reqd: 1,
								default: currentSupplier,
							},
						],
						async (values) => {
							const selectedSupplier = String(values?.supplier || "").trim();
							if (!selectedSupplier) return;
							if (selectedSupplier === currentSupplier) return;
							...
							try {
								...
								// Fetch new supplier's default address and contact
								let customerAddress = "";
								let shippingAddress = "";
								let contactPerson = "";

								try {
									const supplierDoc = await frappe.db.get_doc("Supplier", selectedSupplier);
									...
									shippingAddress = String(
										supplierDoc?.shipping_address_name
										|| supplierDoc?.shipping_address
										|| ""
									).trim();
									...
									// Fallback: query Dynamic Links for Address and Contact
									if (!customerAddress || !shippingAddress || !contactPerson) {
										const linkedRows = await frappe.db.get_list("Dynamic Link", {
											fields: ["parent", "parenttype"],
											filters: {
												link_doctype: "Supplier",
												link_name: selectedSupplier,
												parenttype: ["in", ["Address", "Contact"]],
											},
											limit: 100,
										});
									...
									console.warn("Unable to prefill address/contact for new supplier:", e);
								}

								// Build the set_value payload
								const updateFields = {};
								if (hasQuotationField("supplier")) updateFields.supplier = selectedSupplier;
								if (hasQuotationField("supplier_name")) updateFields.supplier_name = selectedSupplier;
								if (!Object.keys(updateFields).length) updateFields.supplier = selectedSupplier;
								...
								frappe.show_alert({ message: __("Supplier updated"), indicator: "green" }, 4);
								await me.load_doc_details(docname);
							} catch (error) {
								frappe.msgprint(__("Unable to update supplier. Ensure the field allows update after submit and try again."));
							}
						},
						__("Edit Supplier"),
						__("Update")
				...
				.off("click", "#detail-edit-quotation-id-btn")
				.on("click", "#detail-edit-quotation-id-btn", async function (e) {
					e.preventDefault();
					e.stopPropagation();
					...
					const currentDoc = me._currentOpportunityDoc || {};
					const currentQuotationId = String(
						currentDoc.quotation_id
						|| currentDoc.quotation
						|| currentDoc.reference_quotation
						|| ""
					).trim();
						...
						[
							{
								...
								options: "Quotation",
								reqd: 0,
								default: currentQuotationId,
							},
						],
						async (values) => {
							const selectedQuotationId = String(values?.quotation_id || "").trim();
							if (selectedQuotationId === currentQuotationId) return;

							try {
								const candidateFields = ["quotation_id", "quotation", "reference_quotation"];
								let updated = false;
								...
								for (const fieldname of candidateFields) {
									try {
										...
										// Try next known quotation field alias.
									}
								}

								if (!updated) {
									frappe.msgprint(__("Unable to find an editable quotation field on this document."));
									return;
								}

								frappe.show_alert({ message: __("Quotation ID updated"), indicator: "green" }, 4);
								await me.load_doc_details(docname);
							} catch (error) {
								frappe.msgprint(__("Unable to update quotation ID. Ensure the field allows update after submit and try again."));
							}
						},
						__("Edit Quotation ID"),
						__("Update")
					);
				})
		...
		async openQuotationItemSelector(name) {
			...
			const dialog = new frappe.ui.Dialog({
				...
				primary_action: async () => {
					...
					const spqFieldCandidates = await this.getOpportunityItemSpqFieldCandidates().catch(() => ({ rateFields: [], amountFields: [] }));
					const selectedItemsForSupplierQuotation = selectedItems.map((row) => {
						const qty = Number(row?.qty || 1) || 1;
						const rateFromSpq = this.getItemSpqRateNumber(row, row?.rate, spqFieldCandidates?.rateFields || []);
						const amountFromSpq = this.getItemSpqAmountNumber(row, row?.amount || (qty * rateFromSpq), spqFieldCandidates?.amountFields || []);
						return {
							...row,
							qty,
							rate: rateFromSpq,
							amount: amountFromSpq || (qty * rateFromSpq),
							supplier: String(row?.supplier || row?.supplier_name || row?.custom_supplier || "").trim(),
						};
					});

					const supplierFromItems = selectedItemsForSupplierQuotation
						.map((row) => String(row?.supplier || row?.supplier_name || "").trim())
						.find(Boolean) || "";
					...
					const sourceTransactionDate = frappe.datetime.nowdate();
					const sourceValidTill = frappe.datetime.add_days(sourceTransactionDate, 7);

					const pendingQuotationPayload = {
						opportunity: sourceName,
						supplier: supplierFromItems || String(doc?.supplier || doc?.supplier_name || doc?.supplier || doc?.customer || "").trim(),
						customer: String(doc?.supplier || doc?.customer || "").trim(),
						...
						items: selectedItemsForSupplierQuotation,
					};
      ...
      `;
		...
		load_activity(name) {
			...
			frappe.call({
				method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_opportunity_comments",
			...
			$(document)
				...
				.on("click", ".note-edit", function () {
					...
					const d = frappe.prompt(
						...
						function (values) {
							frappe.call({
								method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.update_opportunity_note",
			...
			$(document)
				...
				.on("click", ".note-delete", function (e) {
					...
					frappe.confirm(
						...
						() => {
							frappe.call({
								method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.delete_opportunity_note",
		...
		loadAndRenderNotes(opportunity_name) {
			...
			const self = this;
			frappe.call({
				method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_opportunity_notes",
		...
		bindcallcardsEvents(opportunityName) {
			...
			// ================= FETCH CALLS =================
			frappe.call({
				method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_permitted_call_list",
		...
		`;
			...
			async function loadCallAvatars(list) {
				...
				const res = await frappe.call({
					method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_users_basic_info",
					args: {
			...
			async function getCustomerSalesPerson(customer) {
				...
				const res = await frappe.call({
					method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_customer_sales_person",
					args: { customer },
					silent: true
				});
			...
			async function openNewAppointmentDialog(opportunity_name) {
				...
				try {
					const res = await frappe.call({
						method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_enabled_users",
						silent: true
					});
					users = res.message || [];
				} catch (err) {
		...
		bindAppointmentCardsEvents(opportunityName) {
			...
			frappe.call({
				method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.get_permitted_appointments",
				args: { opportunity_name },
				callback: async function (r) {
		...
		bindOpportunityActivitiesEvents(opportunityName) {
			...
			$('#add-comment-btn').off('click').on('click', async function () {
				...
				frappe.call({
					method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.add_custom_comment",
					args: {
			...
			$('#save-quick-note-btn').off('click').on('click', function () {
				...
				frappe.call({
					method: "renewal_module.custom_module.page.supplier_quotations.supplier_quotations.add_opportunity_note",
					args: {
  ...
  `);
    ...
    `;
		...
		bindFilterEvents() {
			...
			// ========= CONFIG =========
			const DOCTYPE = me.filter_doctype || "Supplier Quotation";
			const LS_KEY = `${DOCTYPE.toLowerCase()}_theme_last_filters`;
			...
			function basicFilters() {
				...
				if (customerValue) F("supplier", "like", `%${customerValue}%`);
	...
	// ---- TEMPLATE ----
	frappe.supplierquotationdata_page_template = {
		body: `
      ...
      <style>
        ...
        .new-opp-table-wrap{
          ...
          overflow-y: auto !important;
				...
				#new-opp-items-table-wrap{
					max-height: none !important;
					overflow-x: visible !important;
					overflow-y: visible !important;
				}

				#new-opp-items-table-wrap .detail-table{
					min-width: 0 !important;
					width: 100% !important;
					table-layout: fixed !important;
				}

				#new-opp-items-table-wrap .detail-table th:nth-child(1),
				#new-opp-items-table-wrap .detail-table td:nth-child(1){ width: auto !important; }
				#new-opp-items-table-wrap .detail-table th:nth-child(2),
				#new-opp-items-table-wrap .detail-table td:nth-child(2){ width: 96px !important; }
				#new-opp-items-table-wrap .detail-table th:nth-child(3),
				#new-opp-items-table-wrap .detail-table td:nth-child(3){ width: 120px !important; }
				#new-opp-items-table-wrap .detail-table th:nth-child(4),
				#new-opp-items-table-wrap .detail-table td:nth-child(4){ width: 132px !important; }
				#new-opp-items-table-wrap .detail-table th:nth-child(5),
				#new-opp-items-table-wrap .detail-table td:nth-child(5){ width: 110px !important; }
				#new-opp-items-table-wrap .detail-table th:nth-child(6),
				#new-opp-items-table-wrap .detail-table td:nth-child(6){
					width: 92px !important;
					min-width: 92px !important;
					max-width: 92px !important;
				}

				#new-opp-items-table-wrap .detail-table thead th,
				#new-opp-items-table-wrap .detail-table tbody td{
					white-space: normal !important;
				}

				#new-opp-items-table-wrap .detail-table .actions-col{
          ...
          <div class="col-12">
            <div class="page-title-head d-flex align-items-center">
              <div class="flex-grow-1">
                <h3 class="fs-xl fw-bold m-0">Supplier Quotation</h3>
              </div>
              <div class="text-end">
            ...
            <div class="controls-wrapper">
              <div class="controls">
                ...
                <div class="btn-group filter-actions">
                  ...
                  <a id="new-opportunity-btn" href="/app/supplier-quotations/new" class="btn btn-sm btn-primary1 mr-2">
                    <i class="fa fa-plus me-1"></i> New Supplier Quotation
                  </a>
                </div>
              </div>
            </div>
...
<div class="quotation-list-details d-none">
  <style>
    ...
    .detail-table th:nth-child(7){ width: 70px; }
    .detail-table th:nth-child(8){ width: 130px; }
    .detail-table th:nth-child(9){ width: 110px; }
    .detail-table th:nth-child(10){ width: 120px; }
    ...
    @media (max-width: 1536px){
      ...
      .detail-table th:nth-child(7){ width: 56px; }
      .detail-table th:nth-child(8){ width: 110px; }
      .detail-table th:nth-child(9){ width: 95px; }
      .detail-table th:nth-child(10){ width: 100px; }
  ...
  <div class="ticket-header detail-page-header">
    <div class="detail-page-title-head">
      <div class="title-left">
        <h3 class="page-title m-0" id="detail-name">Supplier Quotation</h3>
      </div>
      <div class="title-right">
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="javascript:void(0)">CRM</a></li>
          <li class="breadcrumb-item"><a href="/app/supplier-quotations">Supplier Quotation</a></li>
        </ol>
      </div>
    </div>
        ...
        <div class="brand-summary">
          <div class="brand-left">
            <div class="brand">
			  ...
			  <span class="brand__name" id="detail-customer-name">Supplier</span>
			  <button type="button" class="detail-customer-edit-btn" id="detail-edit-customer-btn" title="Edit Supplier">
                <i class="fa fa-pencil"></i>
              </button>
		  ...
		  <!-- Supplier Card -->
				<div class="detail-card info-grid-container">
					...
					<div class="info-grid info-grid--2col" style="margin-top:10px;">
						<div class="info-col">
							<div class="info-row">
								<span class="label">SUPPLIER ADDRESS</span>
					...
					<div class="info-grid info-grid--2col">
						...
						<div class="info-col">
							<div class="info-row" style="margin-top:10px;">
								...
								<div class="value detail-value-with-action">
									<div id="detail-quotation-id">-</div>
									<button type="button" class="detail-customer-edit-btn" id="detail-edit-quotation-id-btn" title="Edit Quotation ID">
										<i class="fa fa-pencil"></i>
									</button>
								</div>
							</div>
        ...
        <div class="new-opportunity-wrap">
          ...
          <div class="new-opp-wizard-panel" data-panel="1">
            <div class="new-opportunity-card">
			  ...
			  <div class="new-opp-field">
                ...
                <div class="new-opp-row two-col">
                  <div class="new-opp-field">
					<label id="new-opp-party-label">Supplier <span style="color:#eb9091">*</span></label>
                    ...
                    <div id="new-opp-company-control" class="new-opp-link-control"></div>
								...
								<div class="new-opp-row two-col">
									<div class="new-opp-field">
										...
										<div id="new-opp-opportunity-id-control" class="new-opp-link-control"></div>
									</div>
									<div class="new-opp-field">
										<label>Quotation ID</label>
										<div id="new-opp-quotation-id-control" class="new-opp-link-control"></div>
									</div>
								</div>
              ...
              <div class="new-opportunity-form">
                <div class="new-opp-field">
                  ...
                  <div id="new-opp-contact-display" class="new-opp-contact-list"><span class="new-opp-contact-empty">No contacts added.</span></div>
								...
								<div class="card__title" style="margin-top:14px;">Company Addresses</div>
								<div class="new-opp-row two-col">
									<div class="new-opp-field">
                          ...
                          <tr>
                            <th><span class="th-ellipsis" title="Item Name">Item Name</span></th>
													 ...
													 <th><span class="th-ellipsis" title="Qty">Qty</span></th>
														...
														<th><span class="th-ellipsis" title="Amount">Amount</span></th>
														<th><span class="th-ellipsis" title="Recommended">Recommended</span></th>
          ...
          <div class="new-opp-wizard-footer">
            ...
            <div class="new-opp-wizard-nav">
              ...
              <button class="btn btn-primary1 d-none" id="new-opp-save" type="button">Create SupplierQuotation</button>
            </div>
          </div>
