...
class calllistspage {
	...
	show_new() {
		...
		this.bind_new_form_events({ customer: prefilledCustomer });
	...
	async bind_new_form_events(opts = {}) {
		...
		$(saveBtn).off("click.newCallList").on("click.newCallList", async function (e) {
			...
			const new_call_list = {
				...
				custom_date,
				description,
				...(prefilledReference ? { reference: prefilledReference } : {}),
				...(prefilledReferenceTo ? { reference_to: prefilledReferenceTo } : {}),
			};
