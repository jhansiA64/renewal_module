...
frappe.pages['support-dashboard-te-1'].on_page_load = function (wrapper) {
	new SupportDashboard(wrapper);
};

// };
frappe.pages['support-dashboard-te-1'].on_page_show = function (wrapper) {
...
class SupportDashboard {
	...
	setActiveSidebar() {
		...
		const baseRoute = route[0]; // "support-dashboard-te-1"
	...
	fetch_ticket_counts() {
		frappe.call({
			method: "renewal_module.custom_module.page.support_dashboard_te_1.support_dashboard_te_1.get_ticket_counts",
			callback: (r) => {
				if (!r.message) return;
	...
	loadIssue() {
		...
		function buildSLAUrl() {
			...
			let url = `/app/ticketss?filters=${filters}`;
		...
		const loadIssues = (append = false) => {
			frappe.call({
				method: "renewal_module.custom_module.page.support_dashboard_te_1.support_dashboard_te_1.get_todays_sla_issues",
				args: {
					start: me.start,
					limit: me.limit
				},
				callback: function (r) {
					...
					data.forEach(issue => {
						const row = `
							<div class="new-row">
								<a href="/app/ticketss/${issue.name}" class="text-decoration-none">
	...
	loadTasks() {
		...
		const loadTask = (append = false) => {
			frappe.call({
				method: "renewal_module.custom_module.page.support_dashboard_te_1.support_dashboard_te_1.todays_due_tasks",
				args: {
					start: me.task_start,
					limit: me.task_limit
				},
	...
	loadCalls() {
		...
		const loadCall = (append = false) => {
			...
			frappe.call({
				method: "renewal_module.custom_module.page.support_dashboard_te_1.support_dashboard_te_1.today_calls",
				args: {
					start: me.call_start,
					limit: me.call_limit
				},
	...
	loadAppointments() {
		...
		const loadAppointment = (append = false) => {
			...
			frappe.call({
				method: "renewal_module.custom_module.page.support_dashboard_te_1.support_dashboard_te_1.today_appointments",
				args: {
					start: me.start,
					limit: me.limit
				},
	...
	loadOems() {
		...
		function buildSLAUrl() {
			...
			if (!frappe.user_roles.includes("System Manager")) {
				...
				url = `/app/ticketss?working_agent=${workingAgent}&filters=${filters}`;
			}
		...
		const loadOem = (append = false) => {
			frappe.call({
				method: "renewal_module.custom_module.page.support_dashboard_te_1.support_dashboard_te_1.get_todays_oem_issues",
				args: {
					start: me.start,
					limit: me.limit
				},
	...
	loadOverdueTickets() {
		...
		const loadOverdue = (append = false) => {
			frappe.call({
				method: "renewal_module.custom_module.page.support_dashboard_te_1.support_dashboard_te_1.overdue_tickets",
				args: {
					offset: me.start,
					limit: me.limit
				},
