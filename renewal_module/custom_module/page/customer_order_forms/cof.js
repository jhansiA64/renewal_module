...
class customerorderformsdatapage {
	...
	async consumePendingQuotationFromOpportunity() {
		...
		this.setNewOpportunityPoAttachment(normalize(payload.po_attachment || payload.customer_po));
	...
	bindNewOpportunityForm() {
		...
		$scope
			...
			.on("click", "#new-opp-save", async function (e) {
				...
				if (poAttachment && hasField("po_attachment")) doc.po_attachment = poAttachment;
				...
				// Set quotation_id if the source doctype is Quotation
				...
				if (sourceDoctype === "Quotation" && sourceOpportunityName && hasField("quotation_id")) {
					doc.quotation_id = sourceOpportunityName;
				}
		...
		if ($poAttachmentHost.length) {
			...
			if (frappe?.ui?.form?.make_control) {
				...
				const attachmentHtml = `
					<div class="frappe-control" data-fieldname="po_attachment" data-fieldtype="Attach">
	...
	async load_doc_details(name) {
		...
		const poAttachment = String(d.po_attachment || d.customer_po || "").trim();
