...
@frappe.whitelist()
def get_list_data(start=0, page_length=20, status="", customer="", probability="", owners=None, filters="[]"):
    """
    ...
    if customer:
        ...
        customer_clauses = []
        if parent_meta.has_field("party_name"):
            customer_clauses.append("ifnull(`party_name`, '') like %(customer)s")
        if parent_meta.has_field("customer"):
            customer_clauses.append("ifnull(`customer`, '') like %(customer)s")
        if customer_clauses:
            conditions.append("(" + " OR ".join(customer_clauses) + ")")
            values["customer"] = customer_like
