...
(() => {
	...
	class customerorderformsdatapage {
		...
		bindDetailMainTabs(defaultTab = "items") {
			const $scope = this.getPageScope();
			const $details = $scope.find(".customer-order-forms-details:visible, .quotation-list-details:visible").first();
			if (!$details.length) return;

			const $tabWrap = $details.find("#detail-main-tabs").first();
			if (!$tabWrap.length) return;

			const $itemSections = $details.find(".detail-main-section--items");
			const $commercialSections = $details.find(".detail-main-section--commercial");

			const setActive = (tabKey) => {
				const normalized = String(tabKey || "").toLowerCase().trim();
				const key = normalized === "commercial" ? "commercial" : "items";
				const showCommercial = key === "commercial";
				this._detailMainActiveTab = key;

				$tabWrap
					.find(".detail-main-tab-btn")
					.removeClass("is-active")
					.filter(`[data-main-tab="${key}"]`)
					.addClass("is-active");

				$itemSections.toggleClass("d-none", showCommercial).toggle(!showCommercial);
				$commercialSections.toggleClass("d-none", !showCommercial).toggle(showCommercial);
			...
			$tabWrap
				.off("click", ".detail-main-tab-btn")
				.on("click", ".detail-main-tab-btn", (e) => {
					e.preventDefault();
					const key = String($(e.currentTarget).attr("data-main-tab") || "items").trim();
					setActive(key);
				});

			setActive(this._detailMainActiveTab || defaultTab);
		}
		...
		async consumePendingQuotationFromOpportunity() {
			...
			this._newOppTotal = Number(payload.total || 0) || 0;
			this._newOppTotalQty = Number(payload.total_quantity || payload.total_qty || 0) || 0;
			this._newOppTotalTaxes = Number(payload.total_taxes_and_charges || 0) || 0;
			...
			const subject = normalize(payload.subject || payload.subject || "");
          ...
          `;
			...
			$scope
				...
				.on("click", "#new-opp-save", async function (e) {
					...
					if (hasField("total") && Number.isFinite(Number(me._newOppTotal))) doc.total = Number(me._newOppTotal) || 0;
					if (Number.isFinite(Number(me._newOppTotalQty))) {
						if (hasField("total_quantity")) doc.total_quantity = Number(me._newOppTotalQty) || 0;
						else if (hasField("total_qty")) doc.total_qty = Number(me._newOppTotalQty) || 0;
					}
					if (hasField("total_taxes_and_charges") && Number.isFinite(Number(me._newOppTotalTaxes))) doc.total_taxes_and_charges = Number(me._newOppTotalTaxes) || 0;
		...
		updateLineItemsFullWidth() {
			const $scope = this.getPageScope();
			const $details = $scope.find(".customer-order-forms-details:visible, .quotation-list-details:visible").first();
			const wrapper = $details.find(".detail-view-wrapper").get(0);
			if (!wrapper) return;
			...
			const lineItemsCard = Array.from(wrapper.querySelectorAll(".detail-card--lineitems"))
				.find((el) => el && el.offsetParent !== null && !el.classList.contains("d-none"));
			if (!panel) {
				wrapper.style.setProperty("--line-items-extra", "0px");
				wrapper.style.setProperty("--line-items-shift", "0px");
				return;
		...
		fetch_list_data({ reset = false, saved_filters = [], override_filters = {} } = {}) {
			...
			me.active_customer = Object.prototype.hasOwnProperty.call(override_filters, "customer")
				? String(override_filters.customer || "").trim()
				: String(me.active_customer || "").trim();
			...
			const customerVal = String(me.active_customer || "").trim();
    ...
    `;
		...
		async load_doc_details(name) {
			...
			const subjectValue = String(d.subject || titleVal || d.name || "").trim();
			const companyDetails = d.company || d.company_name || d.organization || d[OPP_CFG.PARTY_NAME_FIELD] || d.customer || d.lead || "";
			const sourceOpportunity = String(
				d.quotation_id
				|| d.source_opportunity
				|| d.opportunity
				|| d.opportunity_name
				|| d.against_opportunity
				|| d.prevdoc_docname
				|| ""
			).trim();
			...
			const salesTeamRows = Array.isArray(d.sales_team) ? d.sales_team : [];
			const topSalesPeople = salesTeamRows
				.map((row) => String(row?.sales_person || row?.employee || row?.owner || "").trim())
				.filter(Boolean);

			// Update brand/header section
			$scope.find("#detail-customer-name").text(subjectValue || "-").attr("title", subjectValue || "-");
			...
			const $subjectStatus = $scope.find("#detail-subject-status");
			if ($subjectStatus.length) {
				if (detailStatusText && detailStatusText !== "-") {
					$subjectStatus.text(detailStatusText)
						.removeClass("d-none badge--success badge--danger badge--warning badge--orange badge--blue")
						.addClass(detailBadgeClass);
				} else {
					$subjectStatus.text("").addClass("d-none");
				}
			}
			...
			// Update finance summary card in right panel
			const $summarySelling = $scope.find("#summary-total-selling");
			const $summaryPurchase = $scope.find("#summary-total-purchase");
			const $summaryOrc = $scope.find("#summary-total-orc");
			const $summaryProfit = $scope.find("#summary-profit");
			const $summaryTitle = $scope.find("#summary-title");

			if ($summarySelling.length) {
				$summarySelling.text(fmtCurrency(amount));
				$summaryTitle.text(fmtCurrency(amount));
			}
			// Update info grid
			$scope.find("#detail-status").text(statusVal || "-");
			$scope.find("#detail-header-status").text(salesStage || "-");
			const closureText = validTill ? frappe.datetime.str_to_user(validTill) : "";
			$scope.find("#detail-closure-date")
				.text(closureText || "")
				.toggle(!!closureText);
			...
			$scope.find("#detail-hero-customer").text(party || "-").attr("title", party || "-");
			$scope.find("#detail-company-name").text(subjectValue || "-").attr("title", subjectValue || "-");
			$scope.find("#detail-hero-sales-person").text(topSalesPeople.join(", ") || "-").attr("title", topSalesPeople.join(", ") || "-");
			$scope.find("#detail-company-sales-person").text(topSalesPeople.join(", ") || "-").attr("title", topSalesPeople.join(", ") || "-");
			$scope.find("#detail-po-ref-number").text(poRefNumber || "-").attr("title", poRefNumber || "-");
			const avatarText = (subjectValue || party || d.name || "C").trim().charAt(0).toUpperCase() || "C";
			$scope.find("#detail-profile-avatar").text(avatarText);
			...
			const $heroQuotation = $scope.find("#detail-hero-quotation-id");
			...
			if (this._detailLoadRequestToken !== requestToken) return;
			this.updateDetailSummaryCard(d, $scope);
      ...
      `;
		...
		async renderOpportunitySalesTeamSection(doc) {
			const $container = $("#detail-sales-team, #detail-company-sales-person");
			const $heroSales = $("#detail-hero-sales-person");
			if (!$container.length) return;
			...
			if (!teamRows.length) {
				$container.html(`<span class="text-muted">-</span>`);
				$heroSales.text("-").attr("title", "-");
				return;
			}

			const compactSales = teamRows.map((row) => row.sales_person).join(", ");
			$heroSales.text(compactSales || "-").attr("title", compactSales || "-");
      ...
      `;
		...
		async updateDetailSummaryCard(doc, $scope) {
			const $summarySelling = $scope.find("#summary-total-selling");
			const $summaryPurchase = $scope.find("#summary-total-purchase");
			const $summaryOrc = $scope.find("#summary-total-orc");
			const $summaryProfit = $scope.find("#summary-profit");
			...
			if (!$summarySelling.length) return;
			...
			if (!items.length) {
				$summarySelling.text(fmtCurrency(0));
				$summaryPurchase.text(fmtCurrency(0));
				$summaryOrc.text(fmtCurrency(0));
				$summaryProfit.text(fmtCurrency(0));
				$summaryTitle.text(fmtCurrency(0));
				return;
			}

			const spqFieldCandidates = await this.getOpportunityItemSpqFieldCandidates().catch(() => ({
				rateFields: ["buying_rate", "purchase_rate", "custom_buying_rate"],
				amountFields: ["buying_amount", "purchase_amount", "custom_buying_amount"],
			}));
			...
			items.forEach((item) => {
				...
				const amount = asNumber(item?.amount || 0) || (qty * rate);
				totalSelling += amount;

				let purchaseAmount = this.getItemSpqAmountNumber(item, 0, spqFieldCandidates?.amountFields || []);
				if (!(purchaseAmount > 0)) {
					const purchaseRate = this.getItemSpqRateNumber(item, 0, spqFieldCandidates?.rateFields || []);
					purchaseAmount = purchaseRate > 0 ? (qty * purchaseRate) : 0;
				}
				totalPurchase += purchaseAmount;

				const isOrcEnabled = isTruthyFlag(item?.orc) || isTruthyFlag(item?.is_orc);
				if (!isOrcEnabled) return;

				const commissionType = String(item?.commission_type || "").trim().toLowerCase();
				const directCommission = asNumber(item?.commission_amount || item?.orc_amount || 0);
				if (directCommission > 0) {
					totalOrc += directCommission;
					return;
				}

				const rateValue = asNumber(
					item?.rate_value
					|| item?.ratevalue
				...
				if (/unit\s*rate/i.test(commissionType)) {
					totalOrc += (qty * rateValue);
				} else {
					totalOrc += rateValue;
				}
			});

			const totalProfit = totalSelling - totalPurchase - totalOrc;

			$summarySelling.text(fmtCurrency(totalSelling));
			$summaryPurchase.text(fmtCurrency(totalPurchase));
			$summaryOrc.text(fmtCurrency(totalOrc));
			$summaryProfit.text(fmtCurrency(totalProfit));
			$summaryTitle.text(fmtCurrency(totalSelling));
		}

		async renderOrderSummary(doc) {
			const $selling = $("#detail-order-selling-total");
			const $purchase = $("#detail-order-purchase-total");
			const $orc = $("#detail-order-orc-total");
			const $profit = $("#detail-order-profit-total");

			if (!$selling.length || !$purchase.length || !$orc.length || !$profit.length) return;
			...
			const items = Array.isArray(doc?.items) ? doc.items : [];
			if (!items.length) {
				$selling.text(fmtCurrency(0));
				$purchase.text(fmtCurrency(0));
				$orc.text(fmtCurrency(0));
				$profit.text(fmtCurrency(0));
				return;
			}
			...
			$selling.text(fmtCurrency(totalSelling));
			$purchase.text(fmtCurrency(totalPurchase));
			$orc.text(fmtCurrency(totalOrc));
			$profit.text(fmtCurrency(totalProfit));
		}
  ...
  `);
		...
		async renderOpportunityItems(doc) {
			...
			if (!items.length) {
				$itemsTable.html(`<tr><td colspan="13" class="muted">No line items</td></tr>`);
				this.setDetailOrcColumnsVisibility(false);
				this.renderOrderSummary(doc);
				return;
			}
      ...
      <tr data-opportunity-item-name="${frappe.utils.escape_html(oppItemName)}" data-row-name="${frappe.utils.escape_html(rowName)}" data-row-idx="${idx}">
		<td class="idx-col line-items-center">${idx + 1}</td>
        <td>
          <div class="line-item-title">
    ...
    `;
		...
		filterDetailItemsTable(filters = {}) {
			...
			$rows.each(function () {
				const $row = $(this);
				const itemText = String($row.find("td:nth-child(2)").text() || "").trim().toLowerCase();
				const brandText = String($row.find("td:nth-child(3)").text() || "").trim().toLowerCase();
				const opportunityTypeText = String($row.find("td:nth-child(7)").text() || "").trim().toLowerCase();
		...
		/*
				...
				const shownOpportunityType = String($mainRow.find("td:nth-child(7) .cell-ellipsis").text() || "").trim();
			...
			`;
				...
				const applyItemAutofillByCode = async (selectedCode = "", force = false) => {
					...
					$mainRow.find("td:nth-child(3)").html(`<span class="cell-ellipsis" title="${brandText}">${brandText}</span>`);
				};
    ...
    `);
				...
				.on("click", "a[data-action]", async (e) => {
					...
					if (action === "create_customer_order_form") {
						...
						const pendingQuotationPayload = {
							...
							total_quantity: Number(sourceDoc.total_qty || sourceDoc.total_quantity || 0) || 0,
							total: Number(sourceDoc.total || 0) || 0,
							total_taxes_and_charges: Number(sourceDoc.total_taxes_and_charges || sourceDoc.base_total_taxes_and_charges || 0) || 0,
		...
		init_attachment_section(name) {
			...
			if (this._rightPanelCardType === "attachments") {
				this.render_doc_attachments(name, { targetSelector: "#right-panel-attachments-list" });
			}
		}
		...
		open_attachment_uploader(name) {
			...
			new frappe.ui.FileUploader({
				...
				on_success() {
					frappe.show_alert({ message: __("Attachment added"), indicator: "green" });
					// Only update the right panel if we're in attachments popup context
					const targetSelector = me._rightPanelCardType === "attachments" ? "#right-panel-attachments-list" : "";
					me.render_doc_attachments(opportunityName, targetSelector ? { targetSelector } : {});
				}
			});
		}
		...
		async delete_attachment_file(fileName, opportunityName) {
			...
			try {
				...
				// Only update the right panel if we're in attachments popup context
				const targetSelector = this._rightPanelCardType === "attachments" ? "#right-panel-attachments-list" : "";
				this.render_doc_attachments(normalizedOpportunity, targetSelector ? { targetSelector } : {});
			} catch (e) {
    ...
    `;
      ...
      <small class="text-muted">${d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</small>`;
		...
		getStageClass(stage) {
			...
			if (normalized === "draft") return "open1";
			if (normalized.includes("waiting for approval")) return "waiting_approval";
  ...
  <style>
    ...
    .card__title{
      ...
      color: var(--text);
		...
		.info-grid-container--customer .info-row .label{
			font-size: 14px;
		}

		.info-grid-container--customer .info-row .value{
			font-size: 12px;
			min-width: 0;
		}

		.info-grid-container--customer .info-row{
			min-width: 0;
		}

		.info-grid-container--customer #detail-contact-person{
			font-size: 13px;
		}

		.info-grid-container--customer .detail-address-name{
			font-size: 13px;
			display: block;
			width: 100%;
			max-width: 100%;
		...
		.info-grid-container--company .info-row .value{
			font-size: 13px;
		}

		.info-grid-container--po .info-row .label{
			font-size: 14px;
		}

		.info-grid-container--po .info-row .value,
		.info-grid-container--po #detail-po-ref-number,
		.info-grid-container--po #detail-po-attachment{
			font-size: 13px;
		}
    ...
    .brand__name{
      ...
      vertical-align: bottom;
		...
		.hero-meta-item i.hero-icon-customer{
			color: #2563eb;
		}

		.hero-meta-item i.hero-icon-sales-person{
			color: #0f766e;
		}

		.hero-meta-item i.hero-icon-quotation{
			color: #d97706;
		}
    ...
    .info-row .label{
      ...
      letter-spacing: 0.02em;
		...
		.info-row .label .info-label-icon{
			margin-right: 6px;
			width: 10px;
			text-align: center;
		}
    ...
    .detail-table tbody td{
      ...
      text-overflow: ellipsis;
		...
		/* Detailed view: Items table font size */
		.detail-card--items .detail-table thead th,
		.detail-card--items .detail-table tbody td{
			font-size: 13px;
		}

		.detail-card--items .detail-table th.idx-col,
		.detail-card--items .detail-table td.idx-col{
			width: 44px;
			min-width: 44px;
			max-width: 44px;
			padding-left: 4px;
		...
		/* Detail items table: wider Item Name, narrower Opportunity Type */
		.detail-card--items .detail-table th:nth-child(2),
		.detail-card--items .detail-table td:nth-child(2){
			width: 220px;
			min-width: 220px;
		}

		.detail-card--items .detail-table th:nth-child(7),
		.detail-card--items .detail-table td:nth-child(7){
			width: 76px;
			min-width: 76px;
			max-width: 76px;
		}
    ...
    .detail-panel{
      ...
      overflow: auto;
		...
		.panel-finance-summary__row:last-child{
			border-right: none;
			margin-right: 0;
		}

		.panel-finance-summary__row strong{
			color: #0f172a;
			font-size: 16px;
			font-weight: 700;
			font-variant-numeric: tabular-nums;
		...
		.panel-finance-summary__row--profit{
			border-top: none;
			margin-top: 0;
			padding-top: 4px;
		}

		.panel-finance-summary__row--profit strong{
			color: #16a34a;
			font-size: 16px;
		}
    ...
    .opportunities-table .pill.pending{
      ...
      border-color: #c7d2fe;
		...
		.opportunities-table .pill.waiting_approval{
			background: #fef3c7;
			color: #92400e;
			border-color: #fde68a;
		}
	...
	#new-opp-po-attachment-control .form-group .clearfix {
		display:none;
	}
	</style>
  ...
  <div class="ticket-header detail-page-header">
    <div class="detail-page-title-head">
      <div class="title-left">
        <h3 class="page-title m-0" id="detail-customer-name">Customer Order Form</h3>
      </div>
	...
	<div class="detail-view-wrapper">
        ...
        <div class="brand-summary">
					<div class="profile-hero">
						<div class="profile-avatar" id="detail-profile-avatar">C</div>
						<div class="brand-left">
							<div class="brand">
								<span class="brand__name" id="detail-hero-customer">Subject</span>
								<span class="brand__status-inline badge d-none" id="detail-subject-status">-</span>
							</div>
							<div class="hero-meta-row">
								<span class="hero-meta-item">
									<i class="fa fa-briefcase hero-icon-customer"></i>
									<span class="hero-meta-value" id="detail-name">-</span>
								</span>
								<span class="hero-meta-item">
									
									<i class="fa fa-user hero-icon-sales-person"></i>
									<span class="hero-meta-value" id="detail-hero-sales-person">-</span>
								</span>
								<span class="hero-meta-item">
									<i class="fa fa-link hero-icon-quotation"></i>
									<span class="hero-meta-value" id="detail-hero-quotation-id">-</span>
								</span>
							</div>
	          ...
	          <div class="hero-right">
	            ...
	            <div class="hero-actions">
							...
							<div class="dropdown d-none" id="detail-workflow-dropdown">
		...
		<section class="detail-content">
		  ...
		  <!-- Summary Card -->
		  <div class="panel-finance-summary" id="detail-finance-summary">
		    <div class="panel-finance-summary__title">Order Summary</div>
		    <div class="panel-finance-summary__rows">
		      <div class="panel-finance-summary__row">
		        <span>Selling Amount</span>
		        <strong id="summary-total-selling">₹ 0.00</strong>
		      </div>
		      <div class="panel-finance-summary__row">
		        <span>Buying Amount</span>
		        <strong id="summary-total-purchase">₹ 0.00</strong>
		      </div>
		      <div class="panel-finance-summary__row">
		        <span>ORC Amount</span>
		        <strong id="summary-total-orc">₹ 0.00</strong>
		      </div>
		      <div class="panel-finance-summary__row panel-finance-summary__row--profit">
		        <span>Profit</span>
		  ...
		  <!-- Customer Card -->
				<div class="detail-card info-grid-container info-grid-container--customer">
					<div class="card__title card__title--customer">Customer Details</div>
					...
					<div class="info-grid info-grid--2col" style="margin-top:10px;">
						<div class="info-col">
							<div class="info-row">
								<span class="label with-action"><i class="fa fa-user info-label-icon" aria-hidden="true"></i>CONTACT PERSON <i id="add-contact-btn" class="fa fa-plus" title="Add Contact"></i></span>
								<div class="value" id="detail-contact-person">-</div>
							</div>
						</div>
						<div class="info-col">
							<div class="info-row">
								<span class="label"><i class="fa fa-map-marker info-label-icon" aria-hidden="true"></i>CUSTOMER ADDRESS</span>
				...
				<!-- PO Card -->
				<div class="detail-card info-grid-container info-grid-container--po">
					<div class="card__title card__title--customer">PO Details</div>
					<div class="info-grid info-grid--2col" style="margin-top:10px;">
						<div class="info-col">
							<div class="info-row">
						...
						<div class="info-col">
							<div class="info-row">
								<span class="label"><i class="fa fa-paperclip info-label-icon" aria-hidden="true"></i>PO ATTACHMENT</span>
			...
			<div class="detail-main-tabs" id="detail-main-tabs">
				<button type="button" class="detail-main-tab-btn is-active" data-main-tab="items">Items</button>
				<button type="button" class="detail-main-tab-btn" data-main-tab="commercial">Commercial</button>
			</div>

			<!-- Items Card -->
			<div class="detail-card detail-card--fullwidth detail-card--lineitems detail-card--items detail-main-section--items">
        ...
        <div class="table-wrap">
          <table class="detail-table">
            <thead>
              <tr>
				<th class="idx-col line-items-center"><span class="th-ellipsis" title="Idx">Idx</span></th>
        ...
        <datalist id="item-code-suggestions"></datalist>
			...
			<!-- Taxes Card -->
			<div class="detail-card detail-card--fullwidth detail-card--lineitems detail-main-section--commercial d-none">
				<div class="d-flex align-items-center justify-content-between mb-2">
			...
			<div class="detail-card detail-card--fullwidth detail-card--lineitems description-card detail-main-section--commercial d-none">
				<div class="d-flex align-items-center justify-content-between mb-2">
