...
class QuoteListPage {
				...
				new Promise(res => frappe.call({
					...
					callback: r => {
						...
						res(docs.map(c => ({
								...
								owner:   c.owner || "",
								created: this.formatActivityTime(c.creation),
								status:  (c.start_date && c.start_date >= today) ? "scheduled" : "completed",
								meta:    c.start_date
	...
	injectDetailContacts(record) {
		...
		if (me._detailPrimaryIdx === undefined) {
		...
		const renderContacts = () => {
			...
			$list.html(me._detailContacts.map((c, i) => {
				...
				<div style="background:${isPrimary ? "rgba(59,126,248,0.05)" : "#f8fafc"};
					...
					<button class="ql-contact-remove" data-ci="${i}"
					...
					${!isPrimary && c.docname ? `
					<button class="ql-set-primary" data-ci="${i}"
			...
			$list.find(".ql-contact-remove").off("click").on("click", function() {
				const i = parseInt($(this).data("ci"));
				me._showDeleteConfirm(
					me._detailContacts[i]?.name || "this contact",
					() => { me._detailContacts.splice(i, 1); renderContacts(); me._saveDetailContacts(record); }
				);
			});

			$list.find(".ql-set-primary").off("click").on("click", function(e) {
				e.stopPropagation();
				const i = parseInt($(this).data("ci"));
	...
	_openDetailContactPicker(partyName, record, onChange) {
		const me = this;
		$("#qlDConPickerOverlay").remove();
		const existing = new Set((me._detailContacts || []).map(c => String(c.docname || "").trim()));
		...
		frappe.call({
			...
			callback: (r) => {
				const available = (r.message || []).map(c => ({
					docname: c.name,
					name:    [c.first_name, c.last_name].filter(Boolean).join(" ") || c.name,
					email:   c.email_id || "", phone: c.mobile_no || "", role: c.designation || "",
				})).filter(c => !existing.has(String(c.docname).trim()));

				const body = available.length
					? available.map((c, ci) => `
						<div class="ql-dcon-pick" data-ci="${ci}" style="display:flex;align-items:center;gap:12px;padding:12px 14px;border:1.5px solid rgba(0,0,0,0.08);border-radius:10px;cursor:pointer;transition:0.15s;margin-bottom:8px;">
					...
					: `<div style="text-align:center;padding:24px;color:#9ca3af;font-size:12px;"><i class="ti ti-search-off" style="font-size:24px;display:block;margin-bottom:8px;"></i>No more contacts available for this customer.</div>`;
				...
				<div id="qlDConPickerOverlay" style="position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px;">
					<div style="background:#fff;border-radius:16px;width:100%;max-width:520px;max-height:80vh;display:flex;flex-direction:column;box-shadow:0 24px 64px rgba(0,0,0,0.18);">
						<div style="padding:18px 22px 14px;border-bottom:1px solid rgba(0,0,0,0.08);display:flex;align-items:center;justify-content:space-between;">
							...
							<button id="qlDConClose" style="width:30px;height:30px;border-radius:50%;border:1px solid rgba(0,0,0,0.1);background:transparent;color:#9ca3af;cursor:pointer;font-size:15px;display:flex;align-items:center;justify-content:center;"><i class="ti ti-x"></i></button>
						</div>
						<div style="flex:1;overflow-y:auto;padding:16px 22px;">${body}</div>
						<div style="padding:12px 22px;border-top:1px solid rgba(0,0,0,0.08);display:flex;justify-content:flex-end;background:#f8fafc;">
							<button id="qlDConCancel" style="padding:8px 16px;border-radius:6px;border:1px solid rgba(0,0,0,0.1);background:#fff;font-size:13px;cursor:pointer;color:#374151;">Cancel</button>
						</div>
					</div>
				</div>`);
				$("body").append($overlay);
				...
				$overlay.on("click", ".ql-dcon-pick", function() {
					const c = available[parseInt($(this).data("ci"))];
					if (c) {
						me._detailContacts = me._detailContacts || [];
						me._detailContacts.push({ ...c, poc: 0 });
						$overlay.remove();
						onChange();
						me._saveDetailContacts(record);
					}
...
<div class="ql-di-card" data-idx="${idx}">
	...
	injectPaymentSchedule() {
		...
		const template = record.payment_terms_template || "";
		$name.text(template || "");
 
		const schedule = Array.isArray(record.payment_schedule) ? record.payment_schedule : [];
 
		if (!schedule.length) {
			$body.html(`<div style="text-align:center;padding:20px;color:#9ca3af;font-size:12px;">
				${template ? "No payment schedule generated." : "No payment terms set."}
			</div>`);
			return;
		}
		...
		const rows = schedule.map((p, i) => `
			<tr style="border-bottom:1px solid rgba(0,0,0,0.06);">
				...
				<td style="padding:9px 10px;font-size:12px;color:#111827;">${me.escapeAttr(p.payment_term || p.description || "—")}</td>
				<td style="padding:9px 10px;font-size:12px;color:#374151;">${me.formatDate(p.due_date)}</td>
		...
		$body.html(`
			<div style="border:1px solid rgba(0,0,0,0.08);border-radius:10px;overflow:hidden;margin-top:4px;">
				<table style="width:100%;border-collapse:collapse;">
					<thead><tr style="background:#f8fafc;">
						...
						<th style="padding:10px;border-bottom:1px solid rgba(0,0,0,0.08);font-size:11px;font-weight:600;color:#6b7280;text-align:left;">Due Date</th>
						<th style="padding:10px;border-bottom:1px solid rgba(0,0,0,0.08);font-size:11px;font-weight:600;color:#6b7280;text-align:right;">Invoice Portion</th>
	...
	injectDetailActivities() {
		...
		const iconMap  = { Note:"ti-notes", Call:"ti-phone", Task:"ti-checklist", Comment:"ti-message-circle" };
		const colorMap = { Note:"#3b7ef8",  Call:"#16a34a",  Task:"#dc2626",      Comment:"#7c3aed" };
	...
	applyFilter() {
		...
		this.$content.find("#activityList").toggle(!isAttach);
		this.$content.find("#attachmentsPanel").toggleClass("hidden", !isAttach).toggle(isAttach);
		this.$content.find("#actFilter").toggle(isActivity);
		this.$content.find("#actBottom").toggle(!isAttach);
...
this.$content.on("click.qlD", "#qlBtnCreate", (e) => {
	...
	_showPdfPreviewModal(docname) {
			...
			<div style="background:#fff;border-radius:14px;width:100%;max-width:880px;height:92vh;display:flex;flex-direction:column;box-shadow:0 24px 64px rgba(0,0,0,0.28);overflow:hidden;">
				<div style="padding:14px 20px;border-bottom:1px solid rgba(0,0,0,0.08);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
					...
					<div style="display:flex;gap:8px;align-items:center;">
						<button id="qlPdfDownload" style="padding:8px 18px;border-radius:6px;border:none;background:#3b7ef8;color:#fff;font-size:13px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;">
				...
				<div style="flex:1;background:#525659;position:relative;">
					...
					<iframe id="qlPdfFrame" src="${printUrl}" style="width:100%;height:100%;border:none;position:relative;z-index:1;" onload="(function(f){var l=document.getElementById('qlPdfLoading'); if(l) l.style.display='none';})(this)"></iframe>
				</div>
			</div>
		</div>
		...
		$overlay.on("click", "#qlPdfDownload", () => window.open(downloadUrl, "_blank"));
	}
...
["billing", "shipping", "company"].forEach(k => me.renderAddressSlot(k));
		...
		// Payment Terms — preset combo selector
		me.$content.on("click.qlNew", ".ql-pay-combo", (e) => {
			me.selectPayCombo(String($(e.currentTarget).data("combo")));
		});
...
<div class="ql-item-card2 ql-drag-row" data-idx="${idx}" draggable="true">
	...
	autoSetTaxCategory() {
		...
		frappe.call({
			method: "frappe.client.get_value",
			args: { doctype: "Address", filters: { name: billAddr }, fieldname: ["gst_state_number", "gstin"] },
			callback: (ra) => {
	...
	_payComboConfig() {
		return {
			i:   { adv:100, del:0,   after:0,   permission:false },
			ii:  { adv:0,   del:100, after:0,   permission:false },
			iii: { adv:50,  del:25,  after:25,  permission:true  },
			iv:  { adv:50,  del:50,  after:0,   permission:false },
			v:   { adv:0,   del:0,   after:100, permission:true  },
			vi:  { adv:0,   del:50,  after:50,  permission:true  },
		};
	}

	renderPayCombos() {
		const me = this;
		const $wrap = me.$content.find("#qlPayCombos");
		if (!$wrap.length) return;
		const combos = me._payComboConfig();
		$wrap.html(Object.keys(combos).map(key => {
			const c = combos[key];
			const active = (me._payCombo === key);
			const parts = [];
			if (c.adv)   parts.push(`Advance ${c.adv}%`);
			if (c.del)   parts.push(`On Delivery ${c.del}%`);
			if (c.after) parts.push(`After Delivery ${c.after}%`);
			return `
			<div class="ql-pay-combo" data-combo="${key}" style="
				border:${active ? "2px solid #3b7ef8" : "1.5px solid rgba(0,0,0,0.1)"};
				border-radius:12px;padding:14px;cursor:pointer;
				background:${active ? "rgba(59,126,248,0.04)" : "#fff"};
				transition:all .12s;position:relative;">
				${c.permission ? `<span style="position:absolute;top:8px;right:8px;font-size:8px;font-weight:700;color:#d97706;background:rgba(217,119,6,0.12);padding:2px 6px;border-radius:10px;text-transform:uppercase;letter-spacing:.04em;">Approval</span>` : ""}
				<div style="font-family:Syne,sans-serif;font-size:13px;font-weight:700;color:${active ? "#3b7ef8" : "#111827"};margin-bottom:8px;text-transform:capitalize;">Option ${me.escapeAttr(key)}</div>
				${parts.map(p => `<div style="font-size:11px;color:#374151;margin-top:3px;display:flex;align-items:center;gap:5px;"><span style="width:5px;height:5px;border-radius:50%;background:${active ? "#3b7ef8" : "#9ca3af"};flex-shrink:0;"></span>${p}</div>`).join("")}
			</div>`;
		}).join(""));
	}
	...
	selectPayCombo(key) {
		...
		if (!c) return;
		me._payCombo    = key;
		me._payApproved = false;   // reset approval whenever combo changes
		me.renderPayCombos();
		me.$content.find("#qlPayAfterDaysWrap").toggle(c.after > 0);
	...
	_buildPayMilestones(c, baseDate, grandTotal) {
		...
		if (c.del) {
			out.push({ term: "On Delivery", label: "On Delivery", portion: c.del, credit_days: 0, amount: grandTotal * c.del / 100 });
		}
		if (c.after) {
			const days = Number(me._payAfterDays || 0);
	...
	renderPaymentScheduleFromCombo() {
		const me = this;
		const $box = me.$content.find("#qlPaymentSchedule");
		const c = me._payComboConfig()[me._payCombo];
		if (!c) { $box.hide().empty(); return; }

		const grandTotal = (me.wizardItems || []).reduce((s, it) =>
			s + Number(it.qty || 0) * Number(it.sp || 0) * (1 - (Number(it.discount || 0) / 100)), 0);
		const today = frappe.datetime.get_today();
		const fmt   = n => "₹ " + Number(n||0).toLocaleString("en-IN",{minimumFractionDigits:2,maximumFractionDigits:2});
		const milestones = me._buildPayMilestones(c, today, grandTotal);

		// const rows = milestones.map((m, i) => `
		// 	<tr style="border-bottom:1px solid rgba(0,0,0,0.06);">
		// 		<td style="padding:9px 10px;color:#9ca3af;font-size:11px;text-align:center;">${i+1}</td>
		// 		<td style="padding:9px 10px;font-size:12px;color:#111827;">${me.escapeAttr(m.label)}</td>
		// 		<td style="padding:9px 10px;font-size:12px;color:#374151;">${me.escapeAttr(me.formatDate(m.due_date))}</td>
		...
		const rows = milestones.map((m, i) => `
			<tr style="border-bottom:1px solid rgba(0,0,0,0.06);">
				<td style="padding:9px 10px;color:#9ca3af;font-size:11px;text-align:center;">${i+1}</td>
				<td style="padding:9px 10px;font-size:12px;color:#111827;">${me.escapeAttr(m.label)}</td>
				<td style="padding:9px 10px;font-size:12px;color:#374151;">${m.credit_days > 0 ? m.credit_days + " days after invoice" : "On invoice"}</td>
				<td style="padding:9px 10px;font-size:12px;text-align:right;color:#374151;">${m.portion}%</td>
				<td style="padding:9px 10px;font-size:12px;text-align:right;font-weight:700;color:#3b7ef8;">${fmt(m.amount)}</td>
			</tr>`).join("");

		$box.html(`
			<div style="font-size:13px;font-weight:600;color:#111827;margin:16px 0 10px;">Payment Schedule <span style="font-size:11px;font-weight:500;color:#9ca3af;">(estimated on item subtotal)</span></div>
			<div style="border:1px solid rgba(0,0,0,0.08);border-radius:10px;overflow:hidden;">
				<table style="width:100%;border-collapse:collapse;">
					<thead><tr style="background:#f8fafc;">
						<th style="padding:10px;width:42px;border-bottom:1px solid rgba(0,0,0,0.08);font-size:11px;font-weight:600;color:#6b7280;text-align:center;">No.</th>
						<th style="padding:10px;border-bottom:1px solid rgba(0,0,0,0.08);font-size:11px;font-weight:600;color:#6b7280;text-align:left;">Milestone</th>
						<th style="padding:10px;border-bottom:1px solid rgba(0,0,0,0.08);font-size:11px;font-weight:600;color:#6b7280;text-align:left;">Due Date</th>
						<th style="padding:10px;border-bottom:1px solid rgba(0,0,0,0.08);font-size:11px;font-weight:600;color:#6b7280;text-align:right;">Portion</th>
						<th style="padding:10px;border-bottom:1px solid rgba(0,0,0,0.08);font-size:11px;font-weight:600;color:#6b7280;text-align:right;">Amount</th>
					</tr></thead>
					<tbody>${rows}</tbody>
				</table>
			</div>`).show();
	...
	async submitWizard() {
		...
		if (me._payCombo && payCombo && payCombo.permission && !me._payApproved) {
			frappe.msgprint(`Payment term "Option ${me._payCombo}" requires approval. Tick the approval confirmation in the Terms step before submitting.`);
			me.goWizardStep(6);
			return;
		}
		// let paymentSchedule = [];
		...
		if (me._payCombo && payCombo) {
			...
			paymentSchedule = me._buildPayMilestones(payCombo, baseDate, grandForSchedule).map(m => ({
				...
				invoice_portion:     m.portion,
				payment_amount:      m.amount,
				// due_date intentionally omitted — ERPNext computes it from credit_days at invoice time
			}));
		}
			...
			<div class="ql-wiz-panel" id="qlPanel6">
				<div class="card">
					...
					<div style="margin-top:8px;margin-bottom:16px;">
						...
						<div id="qlPayCombos" style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;"></div>
	...
	<button class="btn btn-primary" id="qlSaveItemsBtn" type="button"><i class="ti ti-device-floppy"></i> Save Items</button>
			...
			<div class="card">
				...
				<div id="actFilter" style="display:flex;gap:6px;padding:8px 16px;border-bottom:1px solid rgba(0,0,0,0.05);">
					...
					<div class="act-filter"        data-filter="scheduled" style="padding:3px 12px;font-size:11px;font-weight:600;border-radius:20px;background:#f1f5f9;color:#6b7280;cursor:pointer;border:1px solid rgba(0,0,0,0.08);">Scheduled</div>
