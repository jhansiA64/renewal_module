...
class GrossProfitGenerator(object):
	...
	def process(self):
		...
		for row in reversed(self.si_list):
			...
			if flt(row.margin)>0:
				...
				row.gross_profit = flt(flt(row.orc), self.currency_precision)
			if row.base_amount:
	...
	def set_average_gross_profit(self, new_row):
		new_row.gross_profit = 0
		if new_row.margin > 0 :
			new_row.gross_profit = flt((flt(new_row.margin) - flt(new_row.orc)), self.currency_precision)
		else:
			new_row.gross_profit = flt(flt(new_row.orc), self.currency_precision)
	...
	def load_invoice_items(self):
		...
		if self.filters.get("value"):
			if self.filters.get("value") == "Zero Values":
				conditions += " and `tabSales Invoice Item`.sales_order is Null"
			else:
				conditions += " and `tabSales Invoice Item`.sales_order is Not Null"	
		...
		self.si_list = frappe.db.sql(
			...
			select
				...
				(CASE when tpii.base_net_rate IS NULL and tpoi.base_net_rate  then 0 
				else tpii.base_net_rate END )as purchase_rate,
				(CASE when tpii.base_net_amount IS NULL  then 0 
				else tpii.base_net_amount END )as buying_amount,
				(CASE when tpii.base_net_amount IS NULL  then  0
				else `tabSales Order Item`.base_net_amount - tpii.base_net_amount END )as margin,
				(CASE when toi.orc = 1 then torc.commission_amount
				else 0 END) as orc,
				(CASE 
				when tpii.base_net_amount  IS NULL and torc.commission_amount IS not NULL  then `tabSales Order Item`.amount - torc.commission_amount
				when tpii.base_net_amount  IS NULL and torc.commission_amount IS NULL  then 0
				WHEN tpii.base_net_amount is not NULL and torc.commission_amount IS not NULL then (`tabSales Order Item`.amount - tpii.base_net_amount) - torc.commission_amount
				WHEN tpii.base_net_amount is not NULL and torc.commission_amount IS NULL then `tabSales Order Item`.amount - tpii.base_net_amount
				else 0  END )as profit ,
				tst.sales_person,
				(CASE when tpii.supplier IS NULL then 0
				else tpii.supplier END )as supplier,
				(CASE when tpii.name IS NULL  then 0
				else tpii.name END )as purchase_invoice,
				tu.full_name as user
				{sales_person_cols}
				{payment_term_cols}
			from
			    `tabSales Invoice Item` inner join `tabSales Invoice` on `tabSales Invoice Item`.parent = `tabSales Invoice`.name
				LEFT JOIN `tabSales Order Item`  on `tabSales Invoice Item`.sales_order = `tabSales Order Item`.parent 
                and `tabSales Invoice Item`.so_detail = `tabSales Order Item`.name 
            Left join (SELECT 
				    `tabPurchase Order Item`.item_code, `tabPurchase Order Item`.base_net_rate as base_net_rate, `tabPurchase Order Item`.base_net_amount as base_net_amount,
					tpo.supplier as supplier, tpo.name as name,`tabPurchase Order Item`.qty as qty,`tabPurchase Order Item`.sales_order as sales_order,
					`tabPurchase Order Item`.description as description ,`tabPurchase Order Item`.name as poi_name,
					`tabPurchase Order Item`.sales_order_item as sales_order_item
				    FROM
					    `tabPurchase Order Item` INNER JOIN `tabPurchase Order` tpo on `tabPurchase Order Item`.parent = tpo.name
					WHERE
					     tpo.docstatus != 2	
					GROUP BY `tabPurchase Order Item`.name    
				)as tpoi on tpoi.sales_order = `tabSales Order Item`.parent and `tabSales Order Item`.item_code = tpoi.item_code 
				    and tpoi.qty=`tabSales Order Item`.qty and tpoi.description = `tabSales Order Item`.description
					And `tabSales Order Item`.name = CASE WHEN tpoi.sales_order_item is NULL THEN NULL ELSE tpoi.sales_order_item end				
		    Left join (SELECT 
				    `tabPurchase Invoice Item`.item_code, `tabPurchase Invoice Item`.base_net_rate as base_net_rate, `tabPurchase Invoice Item`.base_net_amount as base_net_amount,
					tpi.supplier as supplier, tpi.name as name,`tabPurchase Invoice Item`.qty as qty,`tabPurchase Invoice Item`.purchase_order as purchase_order,
					`tabPurchase Invoice Item`.description as description ,
					`tabPurchase Invoice Item`.po_detail  as purchase_order_item
				    FROM
					    `tabPurchase Invoice Item` INNER JOIN `tabPurchase Invoice` tpi on `tabPurchase Invoice Item`.parent = tpi.name
					WHERE
					     tpi.docstatus != 2	
					GROUP BY `tabPurchase Invoice Item`.name    
				)as tpii on tpii.purchase_order = tpoi.name and tpoi.item_code = tpii.item_code 
				    and tpii.qty=tpoi.qty and tpii.description = tpii.description
					And tpoi.poi_name = CASE WHEN tpii.purchase_order_item is NULL THEN NULL ELSE tpii.purchase_order_item end				
			Left join `tabQuotation Item` tqi on `tabSales Order Item`.prevdoc_docname = tqi.parent 
				    and `tabSales Order Item`.item_code = tqi.item_code and `tabSales Order Item`.qty = tqi.qty and `tabSales Order Item`.rate = tqi.rate and tqi.description = `tabSales Order Item`.description
			...
			left join `tabORC List` tol on toi2.parent = tol.name
				WHERE tol.docstatus  = 1 and tol.status != "Duplicate") as torc on  torc.opportunity_id = toi.parent and toi.item_code = torc.item_code 
			...
			LEFT JOIN `tabUser` tu on tst.sales_person = tu.full_name
				...
				{payment_term_table}
			where
				`tabSales Invoice`.docstatus=1 and `tabSales Invoice Item`.brand NOT IN ("Google","Microsoft Azure","Amazon Web Services (AWS)") {conditions} {match_cond}
			GROUP BY
