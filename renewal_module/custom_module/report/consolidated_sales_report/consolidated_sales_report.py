...
class GrossProfitGenerator(object):
	...
	def process(self):
		...
		for row in reversed(self.si_list):
			...
			# 	frappe.msgprint("product_bundled")
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql(
			...
			select
				...
				(CASE when tpoi.rate IS NULL  then 0 
				else tpoi.rate END )as buying_rate,
				(CASE when tpoi.amount IS NULL  then 0 
				else tpoi.amount END )as buying_amount
				{sales_person_cols}
				{payment_term_cols}
			from
				`tabSales Invoice` inner join `tabSales Invoice Item`
				on `tabSales Invoice Item`.parent = `tabSales Invoice`.name
				Inner join `tabSales Order Item` tsoi on tsoi.name = `tabSales Invoice Item`.so_detail and tsoi.parent = `tabSales Invoice Item`.sales_order 
                LEFT JOIN `tabPurchase Order Item` tpoi  on tsoi.name = tpoi.sales_order_item and tsoi.parent = tpoi.sales_order	
				join `tabItem` item on item.name = `tabSales Invoice Item`.item_code
				{sales_team_table}
				{payment_term_table}
			where
				`tabSales Invoice`.docstatus=1 and `tabSales Invoice`.is_opening!='Yes' and
				tsoi.brand Not IN ("Bytemail",'Amazon Web Services (AWS)','Microsoft Azure')
				{conditions} {match_cond}
			order by
				`tabSales Invoice`.name desc, `tabSales Invoice`.posting_time desc""".format(
