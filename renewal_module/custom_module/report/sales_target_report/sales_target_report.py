...
class GrossProfitGenerator(object):
	...
	def get_returned_sales_person_items(self):
		...
		returned_sales_persons = frappe.db.sql(
			"""
			select
				si.sales_person as sales_person, si_item.category_type, si_item.category, 
				si_item.target_uom as target_uom, si_item.topline_target as topline_target,
				si_item.bottomline_target
			from
				`tabSales Target` si, `tabSub Target Details List` si_item
			where
