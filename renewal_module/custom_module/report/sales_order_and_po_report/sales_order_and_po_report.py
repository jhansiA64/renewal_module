		...
		self.si_list = frappe.db.sql(
			...
			from
				...
				(SELECT 
					...
					tpo.supplier as supplier, tpo.name as po_id,`tabPurchase Order Item`.qty as qty,`tabPurchase Order Item`.sales_order as sales_order,
					`tabPurchase Order Item`.description as description
				    FROM
					    `tabPurchase Order Item` INNER JOIN `tabPurchase Order` tpo on `tabPurchase Order Item`.parent = tpo.name
					WHERE
					     tpo.docstatus != 2	 
				
				)as tpoi on tpoi.sales_order = `tabSales Order Item`.parent 
				        and `tabSales Order Item`.item_code = tpoi.item_code and tpoi.qty=`tabSales Order Item`.qty and tpoi.description = `tabSales Order Item`.description				
