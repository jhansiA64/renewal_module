...
class GrossProfitGenerator(object):
	...
	def load_invoice_items(self):
		...
		if self.filters.company:
			conditions += " and tsp.company = %(company)s"
		...
		if self.filters.sales_person:
			conditions += """
				and exists(select 1
							...
							where st.parent = tsp.name
							and   st.sales_person = %(sales_person)s)
			"""

		if self.filters.group_by == "Sales Person":
			sales_person_cols = ", sales.sales_person, sales.allocated_amount, sales.incentives"
			sales_team_table = "left join `tabSales Team` sales on sales.parent = tsp.name"
		else:
			sales_person_cols = ""
			sales_team_table = ""

		if self.filters.group_by == "Payment Term":
			payment_term_cols = """,if(tsp.is_return = 1,
										'{0}',
			...
			payment_term_table = """ left join `tabPayment Schedule` schedule on schedule.parent = tsp.name and
																				tsp.is_return = 0 """
		else:
			payment_term_cols = ""
			payment_term_table = ""
		...
		if self.filters.get("sales_invoice"):
			conditions += " and tsp.name = %(sales_invoice)s"

		if self.filters.get("item_code"):
			conditions += " and tstd.item_code = %(item_code)s"
		...
		if self.filters.get("warehouse"):
			...
			if warehouse_details:
				conditions += f" and tstd.warehouse in (select name from `tabWarehouse` wh where wh.lft >= {warehouse_details.lft} and wh.rgt <= {warehouse_details.rgt} and warehouse = wh.name)"
		...
		self.si_list = frappe.db.sql(
			...
			select
				...
				tsp.creation, 
				tstd.category_type,
				tstd.fiscal_year, tstd.description,
				tstd.warehouse, tstd.target_value_bottomline,
				tstd.name as "item_row"
				{sales_person_cols}
				{payment_term_cols}
			from
				`tabSales Person` tsp inner join `tabSub Target Details` tstd
					on tstd.parent = tsp.name
				{sales_team_table}
				{payment_term_table}
			where
				tsp.docstatus=1 and tsp.is_opening!='Yes' {conditions} {match_cond}
			order by
				tsp.posting_date desc, tsp.posting_time desc""".format(
