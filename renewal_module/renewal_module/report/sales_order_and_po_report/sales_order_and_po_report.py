...
class GrossProfitGenerator(object):
	...
	def load_order_items(self):
		...
		if self.filters.get("supplier"):
			conditions += " and tpo.supplier in %(supplier)s"		
		...
		self.si_list = frappe.db.sql(
			"""
				...
				(CASE when tpoi.base_net_amount IS NULL  then  0
				else `tabSales Order Item`.base_net_amount - tpoi.base_net_amount END )as margin,
				(CASE when toi.orc = 1 then torc.commission_amount
				else 0 END) as orc,
				(CASE 
				when tpoi.base_net_amount  IS NULL and torc.commission_amount IS not NULL  then `tabSales Order Item`.amount - torc.commission_amount
				when tpoi.base_net_amount  IS NULL and torc.commission_amount IS NULL  then 0
				WHEN tpoi.base_net_amount is not NULL and torc.commission_amount IS not NULL then (`tabSales Order Item`.amount - tpoi.base_net_amount) - torc.commission_amount
				WHEN tpoi.base_net_amount is not NULL and torc.commission_amount IS NULL then `tabSales Order Item`.amount - tpoi.base_net_amount
				else 0  END )as profit ,
				tst.sales_person,
				(CASE when tpoi.supplier IS NULL then Null
				else tpoi.supplier END )as supplier,
				tpoi.name as purchase_order,
				tu.full_name as user
				{sales_person_cols}
				{payment_term_cols}
			from
			    `tabSales Order Item` left join `tabSales Order` on `tabSales Order Item`.parent = `tabSales Order`.name
				Left join 
				(SELECT 
					...
					`tabPurchase Order Item`.description as description ,`tabPurchase Order Item`.sales_order_item as sales_order_item
				    FROM
		...
		return frappe._dict(
			{
				...
				"purchase_order":product_bundle.purchase_order,
