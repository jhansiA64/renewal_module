...
frappe.query_reports["Renewals Analytics With SQM"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
	...
	"formatter": function(value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "view_button" &&  data && data.indent === 1) {
			// console.log(value)			
			const button_view = `<button class="btn btn-default btn-xs" onclick="frappe.query_reports['Renewals Analytics With SQM'].view_data('${value}')">View</button>`;
			value = button_view;
		}
