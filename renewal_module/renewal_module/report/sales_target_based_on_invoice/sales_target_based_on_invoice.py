...
def execute(filters=None):
    ...
    # frappe.msgprint("<pre>{}</pre>".format(frappe.as_json(sales_data)))
    columns, rows = get_columns(filters), get_category_target_data(filters, sales_data)
...
@frappe.whitelist()
def get_category_target_data(filters, sales_data):
    
    sales_users_data = get_parents_data(filters, "Sales Target")
...
def prepare_data(filters,sales_users_data,sales_user_wise_item_groups,sales_user_wise_brand,sales_data,date_field,sales_field,
    ...
    if filters.get("timespan") != "custom":
        ...
        current_fiscal_year= frappe.db.sql(f""" 
                    SELECT name FROM `tabFiscal Year` tfy 
                    WHERE tfy.year_end_date >= '{date2}' and '{date2}' >= tfy.year_start_date
                """,as_dict=1)
        
        fy = current_fiscal_year[0]["name"]# frappe.msgprint("<pre>{}</pre>".format(frappe.as_json(fiscal_y)))
    ...
    for d in sales_users_data:	
        ...
        if d.get("fiscal_year") == fy:
            ...
            for r in sales_data:
                ...
                if(
                    r.sales_person == d.parent.split("-")[0]
                    ...
                    details["shortfall"] = details.get("bottomline_target") - details.get("achieved_value")
                    # frappe.msgprint("<pre>{}</pre>".format(frappe.as_json(sales_user_wise_item_groups.get(d.parent))))
                elif( r.sales_person == d.parent.split("-")[0] and (not sales_user_wise_brand.get(d.parent) or (d.category_type == "Brand" and r.brand == d.category)) and (d.category_type != "Item Group")):
                    # frappe.msgprint("Hello")
                    # (d.category_type == "Brand" and  r.brand == d.category)
                    if d.target_uom == "Qty":
                        #details["achieved_value"] += r.get("sales_qty", 0)
                        details["achieved_value"] += r.get("sales_qty") or 0
                    else:
                        if d.topline_target == 0:
                            pass
                        else:
                            #details["topline_achieved"]	+= r.get("sales_amount",0)
                            details["topline_achieved"] += r.get("sales_amount") or 0
                        if d.bottomline_target == 0:
                            pass
                        else:
                            #details["achieved_value"] += r.get("profit",0)	
                            details["achieved_value"] += r.get("profit") or 0
                        # if d.topline_target != 0:
                        # details["topline_achieved"] += r.get("sales_amount",0)
                    details["shortfall"] =  details.get("bottomline_target") - details.get("achieved_value")
                    # frappe.msgprint("<pre>{}</pre>".format(frappe.as_json(details)))

                elif( r.sales_person == d.parent.split("-")[0] and (r.brand not in sales_user_wise_brand.get(d.parent))
...
def get_parents_data(filters, partner_doctype):
    ...
    if filters.get("timespan") != "custom":
        ...
        # current_fiscal_year = frappe.get_value('Fiscal Year', {'year_start_date': ['<=', date1], 'year_end_date': ['>=', date1]}, "name")
        current_fiscal_year= frappe.db.sql(f""" 
                    SELECT name FROM `tabFiscal Year` tfy 
                    WHERE tfy.year_end_date >= '{date2}' and '{date2}' >= tfy.year_start_date
                """,as_dict=1)
        ...
        # frappe.msgprint("<pre>{}</pre>".format(frappe.as_json(current_fiscal_year[0]["name"])))
        filters_dict["fiscal_year"] = fiscal_year1        
    if filters.get("sales_person"):
        ...
        # frappe.msgprint("<pre>{}</pre>".format(frappe.as_json(current_fiscal_year[0]["name"])))
        filters_dict["fiscal_year"] = fiscal_year1
        parent = filters.get("sales_person")[0] + "-"+ fiscal_year1
        filters_dict["parent"] = parent
    # frappe.msgprint("<pre>{}</pre>".format(frappe.as_json(filters_dict)))
    # if filters.get("sales_person") :
