...
def get_data(filters):
	return frappe.db.sql(
		...
		SELECT
			...
			`tabOpportunity Item`.custom_committed_date as committed_date,
...
def get_conditions(filters):
	...
	if filters.get("timespan") != "custom":
		...
		if filters.get("based_on") == "Creation":
			conditions.append(f" and DATE(`tabOpportunity`.creation)>='{date1}' and DATE(`tabOpportunity`.creation) <='{date2}'")
		elif filters.get("based_on") == "Committed Date":
			conditions.append(f" and DATE(`tabOpportunity Item`.custom_committed_date) >= '{date1}' and DATE(`tabOpportunity Item`.custom_committed_date) <= '{date2}' ")
		else:
	...
	if filters.get("timespan") == "custom":
		if filters.get("based_on") == "Creation":
			...
			conditions.append(f" and DATE(`tabOpportunity Item`.custom_committed_date) >= %(from_date)s and DATE(`tabOpportunity Item`.custom_committed_date) <= %(to_date)s ")
		else:
			conditions.append(f" and DATE(`tabOpportunity Item`.expected_date) >= %(from_date)s and DATE(`tabOpportunity Item`.expected_date) <= %(to_date)s ")		
